...
target=target||{}).name||target.id||target.serial||uuid(6),logger=eventifierLogger.child({target:targetName}),_(eventApi).functions().forEach(function(method){_.isFunction(target[method])&&eventifierLogger.warn("The target object has already a method named "+method,target),target[method]=function(){var args=[].slice.call(arguments);return eventApi[method].apply(target,args)}}),target}}),define("tpl!taoBooklet/tpl/print",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer=""
;return buffer+='<main class="tao-scope ',buffer+=(0,this.escapeExpression)((helper=helpers.cls)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.cls)?helper.call(depth0,{hash:{},data:data}):helper)+'"></main>'})}),define("taoBooklet/controller/PrintTest/render",["jquery","lodash","core/logger","taoQtiPrint/runner/testRunner","tpl!taoBooklet/tpl/print"],function($,_,loggerFactory,testRunner,printTpl){"use strict";var logger=loggerFactory("printTest/render");function showMessage(msg,type){type=type||"info",$("body").append('<div class="feedback-'+type+'">'+msg+"</div>")}return{start:function(testData,options){
var layoutOptions=options&&options.layout||{},layoutClasses={one_page_item:"one-item-per-page",one_page_section:"one-section-per-page"},timeout=setTimeout(function(){showMessage("Something went wrong...","error"),ready()},45e3),$mainContainer=$(printTpl({cls:_.reduce(layoutOptions,function(list,value,name){return value&&layoutClasses[name]&&list.push(layoutClasses[name]),list},[]).join(" ")}));function ready(){window.status="DONE"}$("body").append($mainContainer),testRunner(testData,options).on("error",function(e){logger.error(e),showMessage(e,"error"),ready()}).on("ready",function(){layoutOptions.add_blank_pages&&function($container){var previous,pages=2,
blankPage=function($section){$section.before('<div class="breaker"></div>'),pages++},computePageFromHeight=function(height){6e3<height&&(pages+=Math.ceil(height/6e3)-1)};$container.children("section").each(function(){var height,$section=$(this);$section.hasClass("section")&&(pages%2==0&&blankPage($section),computePageFromHeight($section.outerHeight()),previous="section",pages++),$section.hasClass("item")&&("item"!==previous&&(pages%2!=0&&blankPage($section),height=$section.outerHeight(),$section.nextUntil(".section",".item").each(function(){height+=$(this).outerHeight()}),computePageFromHeight(height),pages++),previous="item")})}($mainContainer),
clearTimeout(timeout),ready()}).render($mainContainer)}}}),function(global,factory){"object"==typeof exports&&"undefined"!=typeof module?module.exports=factory(require("lodash"),require("core/promise"),require("core/eventifier"),require("lib/uuid")):"function"==typeof define&&define.amd?define("core/promiseQueue",["lodash","core/promise","core/eventifier","lib/uuid"],factory):(global=global||self)["core/promiseQueue"]=factory(global._,global["core/promise"],global["core/eventifier"],global["lib/uuid"])}(this,function(_,Promise,eventifier,uuid){"use strict";return _=_&&_.hasOwnProperty("default")?_.default:_,Promise=Promise&&Promise.hasOwnProperty("default"
