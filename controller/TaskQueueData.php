...
class TaskQueueData extends tao_actions_CommonModule
{
    ...
    /**
     * Downloads the PDF attached to the generated booklet
        ...
        if ($this->hasRequestParameter('taskId')) {
            ...
            if (!is_null($report)) {
                ...
                foreach ($report->getSuccesses() as $success) {
                    if (!is_null($instance = $success->getData())) {
                        break;
                    }
                }

                if (is_array($instance)) {
                    if (isset($instance['uriResource'])) {
                        $instance = new core_kernel_classes_Resource($instance['uriResource']);
                    } else {
                        throw new \common_exception_InconsistentData('The report does not contain a valid Booklet instance!');
                    }
                }
                ...
                if ($fileResource instanceof core_kernel_classes_Resource) {
                    ...
                    tao_helpers_Http::returnFile( $file->getAbsolutePath() );
                }
            }
        }
        $this->returnJson([
