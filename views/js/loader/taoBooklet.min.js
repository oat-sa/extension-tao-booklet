...
result.task.report.children[0].data.uriResource)):(this.displayReport(result.task.report.children[0],__("Error")),switchContainer("report")))}).on("continue",function(){refreshTree()}).on("error",function(err){taskCreationButton.terminate().reset(),this.displayReport(err,__("Error")),switchContainer("report")}).on("enqueued",function(){refreshTree()}).render($submitter.closest(".form-toolbar")),$submitter.replaceWith(taskCreationButton.getElement().css({float:"right"}))}}}),define("taoBooklet/controller/routes",[],function(){"use strict";return{Booklet:{actions:{editBooklet:"controller/Booklet/editBooklet",wizard:"controller/Booklet/wizard",
