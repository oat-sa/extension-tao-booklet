define("taoBooklet/controller/routes",[],function(){"use strict";return{Booklet:{actions:{editBooklet:"controller/Booklet/editBooklet",wizard:"controller/Booklet/wizard",preview:"controller/Booklet/preview"}}}}),define("taoBooklet/controller/Booklet/editBooklet",["jquery","lodash","module","helpers","generis.tree.select","layout/section"],function($,_,module,helpers,GenerisTreeSelectClass,section){"use strict";function buildGroupsTree($container){var treeOptions=$container.data(),toggleTree=function(val){$container.toggle(val!==treeOptions.anonymous)},$radio=$('[name="'+treeOptions.anonymousClass+'"]');toggleTree($radio.filter(":checked").val()),$radio.on("change",function(e){
toggleTree($(e.target).val())}),new GenerisTreeSelectClass("#"+treeOptions.actionId+"-tree",helpers._url("getData","GenerisTree","tao"),{actionId:treeOptions.actionId,saveUrl:treeOptions.saveUrl,saveData:{resourceUri:treeOptions.resourceUri,propertyUri:treeOptions.propertyUri},checkedNodes:treeOptions.checkedNodes,serverParameters:{openNodes:treeOptions.openNodes,rootNode:treeOptions.rootNode},saveCallback:function(){section.current().loadContentBlock(helpers._url("editBooklet","Booklet","taoBooklet"),{uri:treeOptions.resourceUri})},paginate:10})}return{start:function(){var $downloader=$("<iframe/>").hide();$("form").append($downloader),$(".btn-download").on("click",function(e){
e.preventDefault();var $form=$(e.target).closest("form");$downloader.attr("src",helpers._url("download","Booklet","taoBooklet",{uri:$form.find('input[name="uri"]').val(),classUri:$form.find('input[name="classUri"]').val(),id:$form.find('input[name="id"]').val()}))}),buildGroupsTree($(".group-container"))}}}),define("taoBooklet/controller/Booklet/preview",["jquery"],function($){"use strict";return{start:function(){var $frame=$(".preview-test"),$container=$frame.closest(".content-container");$frame.css("min-height",$container.height()-50+"px")}}}),define("taoBooklet/controller/Booklet/wizard",["jquery"],function($){"use strict";return{start:function(){var $form=$(".main-container ").find("form"),$radio=$('[name="'+$form.find("#anonymousClass").val()+'"]');
$radio.is(":checked")||$radio.filter(":first").attr("checked",!0)}}}),define("taoItems/assets/manager",["lodash","util/url"],function(_,urlUtil){"use strict";var assetManagerFactory=function(strategies,data,options){var cache={};strategies=_.isArray(strategies)?strategies:[strategies],data=data||{},options=_.defaults(options||{},{parseUrl:!0});var assetManager={_strategies:[],addStrategy:function(strategy){if(!_.isPlainObject(strategy)||!_.isFunction(strategy.handle)||!_.isString(strategy.name))throw new TypeError("An asset resolution strategy is an object with a handle method and a name");this._strategies.push(strategy)},setStrategies:function(newStrategies){
...
suffix="_"+i,i++);while(exists);return id},buildIdentifier:function(item,prefix,useSuffix){var id,usedIds,suffix="",i=1,exists=!1;if(!item)throw new TypeError("A item is required to generate a unique identifier");if(!prefix)throw new TypeError("Prefix is required to build an identifier");usedIds=item.getUsedIdentifiers(),useSuffix=_.isUndefined(useSuffix)?!0:useSuffix,prefix&&(prefix=prefix.replace(/_[0-9]+$/gi,"_").replace(/[^a-zA-Z0-9_]/gi,"_").replace(/(_)+/gi,"_"),useSuffix&&(suffix="_"+i));do exists=!1,id=prefix+suffix,usedIds[id]&&(exists=!0,suffix="_"+i,i++);while(exists);return id},findInCollection:function(element,collectionNames,searchedSerial){var found=null;
if(_.isString(collectionNames)&&(collectionNames=[collectionNames]),!_.isArray(collectionNames))throw new Error("invalid argument : collectionNames must be an array or a string");return _.each(collectionNames,function(collectionName){var collection=element;_.each(collectionName.split("."),function(nameToken){collection=collection[nameToken]});var elt=collection[searchedSerial];return elt?(found={parent:element,element:elt},!1):(_.each(collection,function(elt){return _.isFunction(elt.find)&&(found=elt.find(searchedSerial))?!1:void 0}),found?!1:void 0)}),found},addMarkupNamespace:function(markup,ns){return ns?markup=markup.replace(/<(\/)?([a-z:]+)(\s?)([^><]*)>/g,function($0,$1,$2,$3,$4){
return $2.indexOf(":")>0?$0:($1=$1||"",$3=$3||"","<"+$1+ns+":"+$2+$3+$4+">")}):markup},removeMarkupNamespaces:function(markup){return markup.replace(/<(\/)?(\w*):([^>]*)>/g,"<$1$3>")},getMarkupUsedNamespaces:function(markup){var namespaces=[];return markup.replace(/<(\/)?(\w*):([^>]*)>/g,function(original,slash,ns,node){return namespaces.push(ns),"<"+slash+node+">"}),_.uniq(namespaces)}};return util}),define("taoQtiItem/qtiItem/helper/rendererConfig",["lodash","jquery"],function(_,$){var rendererConfigHelper={};return rendererConfigHelper.getOptionsFromArguments=function(args){var options={data:{},placeholder:null,subclass:"",renderer:null};return _.each(args,function(arg){
arg&&(arg.isRenderer?options.renderer=arg:arg instanceof $&&arg.length?options.placeholder=arg:_.isString(arg)?options.subclass=arg:_.isPlainObject(arg)?options.data=arg:console.log("invalid arg",arg,args))}),options},rendererConfigHelper}),define("taoQtiItem/qtiItem/core/Element",["jquery","lodash","class","taoQtiItem/qtiItem/helper/util","taoQtiItem/qtiItem/helper/rendererConfig"],function($,_,Class,util,rendererConfig){"use strict";var _instances={},Element=Class.extend({qtiClass:"",serial:"",relatedItem:null,init:function(serial,attributes){if(this.attributes={},this.metaData={},"object"==typeof serial&&(attributes=serial,serial=""),serial||(serial=util.buildSerial(this.qtiClass+"_")),
serial&&("string"!=typeof serial||!serial.match(/^[a-z_0-9]*$/i)))throw"invalid QTI serial : ("+typeof serial+") "+serial;if(_instances[serial])throw"a QTI Element with the same serial already exists "+serial;_instances[serial]=this,this.serial=serial,this.setAttributes(attributes||{}),"function"==typeof this.initContainer&&this.initContainer(arguments[2]||""),"function"==typeof this.initObject&&this.initObject()},is:function(qtiClass){return qtiClass===this.qtiClass},placeholder:function(){return"{{"+this.serial+"}}"},getSerial:function(){return this.serial},getUsedIdentifiers:function(){var usedIds={},elts=this.getComposingElements();for(var i in elts){var elt=elts[i],id=elt.attr("identifier");
id&&(usedIds[id]=elt)}return usedIds},getUsedIds:function(){var usedIds=[];return _.forEach(this.getComposingElements(),function(elt){var id=elt.attr("id");id&&!_.contains(usedIds,id)&&usedIds.push(id)}),usedIds},attr:function(name,value){if(name)if(void 0!==value)this.attributes[name]=value;else if("object"==typeof name)for(var prop in name)this.attr(prop,name[prop]);else if("string"==typeof name)return void 0===this.attributes[name]?void 0:this.attributes[name];return this},data:function(name,value){if(name)if(void 0!==value)this.metaData[name]=value,$(document).trigger("metaChange.qti-widget",{element:this,key:name,value:value});else if("object"==typeof name)for(var prop in name)this.data(prop,name[prop]);else if("string"==typeof name)return void 0===this.metaData[name]?void 0:this.metaData[name];
...
},getRenderer:function(){return this.renderer},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),_renderer=args.renderer||this.getRenderer(),tplData={},defaultData={tag:this.qtiClass,serial:this.serial,attributes:this.getAttributes()};if(!_renderer)throw"render: no renderer found for the element "+this.qtiClass+":"+this.serial;"function"==typeof this.initContainer&&(defaultData.body=this.getBody().render(args.renderer)),"function"==typeof this.initObject&&(defaultData.object={attributes:this.object.getAttributes()},defaultData.object.attributes.data=_renderer.resolveUrl(this.object.attr("data"))),tplData=_.merge(defaultData,args.data||{}),
tplData=_renderer.getData(this,tplData,args.subclass);var rendering=_renderer.renderTpl(this,tplData,args.subclass);return args.placeholder&&args.placeholder.replaceWith(rendering),rendering},postRender:function(data,altClassName,renderer){var postRenderers=[],_renderer=renderer||this.getRenderer();if("function"==typeof this.initContainer&&(postRenderers=this.getBody().postRender({},"",renderer)),!_renderer)throw"postRender: no renderer found for the element "+this.qtiClass+":"+this.serial;return postRenderers.push(_renderer.postRender(this,data,altClassName)),_.compact(postRenderers)},getContainer:function($scope,subclass){var renderer=this.getRenderer();if(renderer)return renderer.getContainer(this,$scope,subclass);
...
Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))};return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoItems/assets/strategies",["util/url"],function(urlUtil){"use strict";var prependToUrl=function(url,base,slashcat){return slashcat===!0?base.replace(/\/$/,"")+"/"+url.directory.replace(/^\.\//,"").replace(/^\//,"")+encodeURIComponent(url.file.replace(/^\.\//,"").replace(/^\//,"")):base+url.directory.replace(/^\.?\//,"")+encodeURIComponent(url.file.replace(/^\.?\//,""))},strategies={baseUrl:{name:"baseUrl",handle:function(url,data){return"string"==typeof data.baseUrl&&urlUtil.isRelative(url)?prependToUrl(url,data.baseUrl,data.slashcat):void 0;
}},itemCssNoCache:{name:"itemCssNoCache",handle:function(url,data){return"string"==typeof data.baseUrl&&urlUtil.isRelative(url)&&/\.css$/.test(url.file)?urlUtil.build(prependToUrl(url,data.baseUrl,data.slashcat),{bust:Date.now()}):void 0}},external:{name:"external",handle:function(url){return urlUtil.isAbsolute(url)?url.toString():void 0}},base64:{name:"base64",handle:function(url){return urlUtil.isBase64(url)?url.toString():void 0}},taomedia:{name:"taomedia",handle:function(url,data){var baseUrl=data.baseUrl||"./";return"object"==typeof url&&"taomedia"===url.protocol||/^taomedia:\/\//.test(url.toString())?baseUrl+url.toString():void 0}}};return strategies}),
define("taoQtiPrint/qtiPrintRenderer/renderers/config",["taoItems/assets/manager","taoItems/assets/strategies"],function(assetManagerFactory,assetStrategies){"use strict";var assetManager=assetManagerFactory([assetStrategies.taomedia,assetStrategies.external,assetStrategies.base64,assetStrategies.baseUrl],{baseUrl:""});return{name:"printRenderer",locations:{assessmentItem:"taoQtiPrint/qtiPrintRenderer/renderers/Item",_container:"taoQtiPrint/qtiPrintRenderer/renderers/Container",stylesheet:"taoQtiPrint/qtiPrintRenderer/renderers/Stylesheet",img:"taoQtiPrint/qtiPrintRenderer/renderers/Img",rubricBlock:"taoQtiPrint/qtiPrintRenderer/renderers/RubricBlock",object:"taoQtiPrint/qtiPrintRenderer/renderers/Object",
prompt:"taoQtiPrint/qtiPrintRenderer/renderers/interactions/Prompt",choiceInteraction:"taoQtiPrint/qtiPrintRenderer/renderers/interactions/ChoiceInteraction",matchInteraction:"taoQtiPrint/qtiPrintRenderer/renderers/interactions/MatchInteraction",inlineChoiceInteraction:"taoQtiPrint/qtiPrintRenderer/renderers/interactions/InlineChoiceInteraction",extendedTextInteraction:"taoQtiPrint/qtiPrintRenderer/renderers/interactions/ExtendedTextInteraction",textEntryInteraction:"taoQtiPrint/qtiPrintRenderer/renderers/interactions/TextEntryInteraction","simpleChoice.choiceInteraction":"taoQtiPrint/qtiPrintRenderer/renderers/choices/SimpleChoice.ChoiceInteraction","simpleAssociableChoice.matchInteraction":"taoQtiPrint/qtiPrintRenderer/renderers/choices/SimpleAssociableChoice.MatchInteraction",
inlineChoice:"taoQtiPrint/qtiPrintRenderer/renderers/choices/InlineChoice",math:"taoQtiPrint/qtiPrintRenderer/renderers/Math",_simpleFeedbackRule:!1,outcomeDeclaration:!1,responseDeclaration:!1,responseProcessing:!1,modalFeedback:!1,orderInteraction:!1,associateInteraction:!1,sliderInteraction:!1,"simpleChoice.orderInteraction":!1,hottext:!1,gap:!1,gapText:!1,"simpleAssociableChoice.associateInteraction":!1,hottextInteraction:!1,hotspotInteraction:!1,gapMatchInteraction:!1,selectPointInteraction:!1,graphicOrderInteraction:!1,mediaInteraction:!1,uploadInteraction:!1,graphicGapMatchInteraction:!1,gapImg:!1,graphicAssociateInteraction:!1,customInteraction:!1,infoControl:!1,
endAttemptInteraction:!1},options:{assetManager:assetManager}}}),define("taoQtiPrint/qtiPrintRenderer/renderers/Renderer",["taoQtiItem/qtiRunner/core/Renderer","taoQtiPrint/qtiPrintRenderer/renderers/config","css!taoQtiPrintCss/qti.css"],function(Renderer,config){"use strict";return Renderer.build(config.locations,config.name,config.options)}),define("tpl!taoQtiPrint/qtiPrintRenderer/tpl/notYetSupported",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n    <div class="additional">\n        <div class="title">Additional information:</div>\n        <p class="info">',
(helper=helpers.additional)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.additional,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+="</p>\n    </div>\n"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",self=this;return buffer+='<div class="render-error feedback-error">\n    <span class="icon-error"></span>\n    <p class="message">',(helper=helpers.message)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.message,stack1=typeof helper===functionType?helper.call(depth0,{
hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+="</p>\n",stack1=helpers["if"].call(depth0,depth0&&depth0.additional,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n</div>\n"})}),define("taoQtiPrint/runner/provider/qtiprint",["i18n","helpers","core/logger","taoQtiItem/qtiItem/core/Loader","taoQtiPrint/qtiPrintRenderer/renderers/Renderer","tpl!taoQtiPrint/qtiPrintRenderer/tpl/notYetSupported"],function(__,helpers,loggerFactory,QtiLoader,QtiRenderer,notYetSupportedTpl){"use strict";function missingSupport(item,additional){var label=item.identifier;return item.attributes&&item.attributes.label&&(label=item.attributes.label),
notYetSupportedTpl({message:__("The item &laquo;%s&raquo; contains an interaction that is not yet supported for printing purpose",label),additional:additional})}var logger=loggerFactory("runner/provider/qtiPrint"),qtiItemRuntimeProvider={init:function(itemData,done){var self=this;this._renderer=new QtiRenderer({assetManager:this.assetManager}),(new QtiLoader).loadItemData(itemData,function(item){if(!item)return self.trigger("error","Unable to load item from the given data.");self._itemData=itemData,self._item=item;try{self._renderer.load(function(){self._item.setRenderer(this),done()},this.getLoadedClasses())}catch(e){self.trigger("error","Unable to render the item : "+e);
}})},render:function(elt,done){var self=this;if(this._item){try{elt.innerHTML=this._item.render({})}catch(e){logger.trace(e),elt.innerHTML=missingSupport(this._itemData,e),self.trigger("warning","Error in template rendering : "+e)}try{this._item.postRender()}catch(e){logger.error(e),self.trigger("error","Error in post rendering : "+e)}done()}},clear:function(elt,done){done()},getState:function(){return{}},setState:function(state){},getResponses:function(){return[]}};return qtiItemRuntimeProvider}),define("taoQtiPrint/runner/itemRunner",["taoItems/runner/api/itemRunner","taoQtiPrint/runner/provider/qtiprint"],function(itemRunner,qtiRuntimeProvider){"use strict";
return itemRunner.register("qtiprint",qtiRuntimeProvider),itemRunner}),define("taoQtiPrint/lib/qrcode",[],function(){function QR8bitByte(data){this.mode=QRMode.MODE_8BIT_BYTE,this.data=data,this.parsedData=[];for(var i=0,l=this.data.length;l>i;i++){var byteArray=[],code=this.data.charCodeAt(i);code>65536?(byteArray[0]=240|(1835008&code)>>>18,byteArray[1]=128|(258048&code)>>>12,byteArray[2]=128|(4032&code)>>>6,byteArray[3]=128|63&code):code>2048?(byteArray[0]=224|(61440&code)>>>12,byteArray[1]=128|(4032&code)>>>6,byteArray[2]=128|63&code):code>128?(byteArray[0]=192|(1984&code)>>>6,byteArray[1]=128|63&code):byteArray[0]=code,this.parsedData.push(byteArray)}this.parsedData=Array.prototype.concat.apply([],this.parsedData),
...
this.setupPositionAdjustPattern(),this.setupTimingPattern(),this.setupTypeInfo(test,maskPattern),this.typeNumber>=7&&this.setupTypeNumber(test),null==this.dataCache&&(this.dataCache=QRCodeModel.createData(this.typeNumber,this.errorCorrectLevel,this.dataList)),this.mapData(this.dataCache,maskPattern)},setupPositionProbePattern:function(row,col){for(var r=-1;7>=r;r++)if(!(-1>=row+r||this.moduleCount<=row+r))for(var c=-1;7>=c;c++)-1>=col+c||this.moduleCount<=col+c||(r>=0&&6>=r&&(0==c||6==c)||c>=0&&6>=c&&(0==r||6==r)||r>=2&&4>=r&&c>=2&&4>=c?this.modules[row+r][col+c]=!0:this.modules[row+r][col+c]=!1)},getBestMaskPattern:function(){for(var minLostPoint=0,pattern=0,i=0;8>i;i++){
this.makeImpl(!0,i);var lostPoint=QRUtil.getLostPoint(this);(0==i||minLostPoint>lostPoint)&&(minLostPoint=lostPoint,pattern=i)}return pattern},createMovieClip:function(target_mc,instance_name,depth){var qr_mc=target_mc.createEmptyMovieClip(instance_name,depth),cs=1;this.make();for(var row=0;row<this.modules.length;row++)for(var y=row*cs,col=0;col<this.modules[row].length;col++){var x=col*cs,dark=this.modules[row][col];dark&&(qr_mc.beginFill(0,100),qr_mc.moveTo(x,y),qr_mc.lineTo(x+cs,y),qr_mc.lineTo(x+cs,y+cs),qr_mc.lineTo(x,y+cs),qr_mc.endFill())}return qr_mc},setupTimingPattern:function(){for(var r=8;r<this.moduleCount-8;r++)null==this.modules[r][6]&&(this.modules[r][6]=r%2==0);
for(var c=8;c<this.moduleCount-8;c++)null==this.modules[6][c]&&(this.modules[6][c]=c%2==0)},setupPositionAdjustPattern:function(){for(var pos=QRUtil.getPatternPosition(this.typeNumber),i=0;i<pos.length;i++)for(var j=0;j<pos.length;j++){var row=pos[i],col=pos[j];if(null==this.modules[row][col])for(var r=-2;2>=r;r++)for(var c=-2;2>=c;c++)-2==r||2==r||-2==c||2==c||0==r&&0==c?this.modules[row+r][col+c]=!0:this.modules[row+r][col+c]=!1}},setupTypeNumber:function(test){for(var bits=QRUtil.getBCHTypeNumber(this.typeNumber),i=0;18>i;i++){var mod=!test&&1==(bits>>i&1);this.modules[Math.floor(i/3)][i%3+this.moduleCount-8-3]=mod}for(var i=0;18>i;i++){var mod=!test&&1==(bits>>i&1);
this.modules[i%3+this.moduleCount-8-3][Math.floor(i/3)]=mod}},setupTypeInfo:function(test,maskPattern){for(var data=this.errorCorrectLevel<<3|maskPattern,bits=QRUtil.getBCHTypeInfo(data),i=0;15>i;i++){var mod=!test&&1==(bits>>i&1);6>i?this.modules[i][8]=mod:8>i?this.modules[i+1][8]=mod:this.modules[this.moduleCount-15+i][8]=mod}for(var i=0;15>i;i++){var mod=!test&&1==(bits>>i&1);8>i?this.modules[8][this.moduleCount-i-1]=mod:9>i?this.modules[8][15-i-1+1]=mod:this.modules[8][15-i-1]=mod}this.modules[this.moduleCount-8][8]=!test},mapData:function(data,maskPattern){for(var inc=-1,row=this.moduleCount-1,bitIndex=7,byteIndex=0,col=this.moduleCount-1;col>0;col-=2)for(6==col&&col--;;){
for(var c=0;2>c;c++)if(null==this.modules[row][col-c]){var dark=!1;byteIndex<data.length&&(dark=1==(data[byteIndex]>>>bitIndex&1));var mask=QRUtil.getMask(maskPattern,row,col-c);mask&&(dark=!dark),this.modules[row][col-c]=dark,bitIndex--,-1==bitIndex&&(byteIndex++,bitIndex=7)}if(row+=inc,0>row||this.moduleCount<=row){row-=inc,inc=-inc;break}}}},QRCodeModel.PAD0=236,QRCodeModel.PAD1=17,QRCodeModel.createData=function(typeNumber,errorCorrectLevel,dataList){for(var rsBlocks=QRRSBlock.getRSBlocks(typeNumber,errorCorrectLevel),buffer=new QRBitBuffer,i=0;i<dataList.length;i++){var data=dataList[i];buffer.put(data.mode,4),buffer.put(data.getLength(),QRUtil.getLengthInBits(data.mode,typeNumber)),
...
for(var col=0;moduleCount>col;col++)for(var row=0;moduleCount-6>row;row++)qrCode.isDark(row,col)&&!qrCode.isDark(row+1,col)&&qrCode.isDark(row+2,col)&&qrCode.isDark(row+3,col)&&qrCode.isDark(row+4,col)&&!qrCode.isDark(row+5,col)&&qrCode.isDark(row+6,col)&&(lostPoint+=40);for(var darkCount=0,col=0;moduleCount>col;col++)for(var row=0;moduleCount>row;row++)qrCode.isDark(row,col)&&darkCount++;var ratio=Math.abs(100*darkCount/moduleCount/moduleCount-50)/5;return lostPoint+=10*ratio}},QRMath={glog:function(n){if(1>n)throw new Error("glog("+n+")");return QRMath.LOG_TABLE[n]},gexp:function(n){for(;0>n;)n+=255;for(;n>=256;)n-=255;return QRMath.EXP_TABLE[n]},EXP_TABLE:new Array(256),
LOG_TABLE:new Array(256)},i=0;8>i;i++)QRMath.EXP_TABLE[i]=1<<i;for(var i=8;256>i;i++)QRMath.EXP_TABLE[i]=QRMath.EXP_TABLE[i-4]^QRMath.EXP_TABLE[i-5]^QRMath.EXP_TABLE[i-6]^QRMath.EXP_TABLE[i-8];for(var i=0;255>i;i++)QRMath.LOG_TABLE[QRMath.EXP_TABLE[i]]=i;QRPolynomial.prototype={get:function(index){return this.num[index]},getLength:function(){return this.num.length},multiply:function(e){for(var num=new Array(this.getLength()+e.getLength()-1),i=0;i<this.getLength();i++)for(var j=0;j<e.getLength();j++)num[i+j]^=QRMath.gexp(QRMath.glog(this.get(i))+QRMath.glog(e.get(j)));return new QRPolynomial(num,0)},mod:function(e){if(this.getLength()-e.getLength()<0)return this;
...
this._oQRCode.make(),this._el.title=sText,this._oDrawing.draw(this._oQRCode),this.makeImage()},QRCode.prototype.makeImage=function(){"function"==typeof this._oDrawing.makeImage&&(!this._android||this._android>=3)&&this._oDrawing.makeImage()},QRCode.prototype.clear=function(){this._oDrawing.clear()},QRCode.CorrectLevel=QRErrorCorrectLevel,QRCode}),define("taoQtiPrint/runner/testRunner",["jquery","lodash","async","helpers","taoQtiPrint/runner/itemRunner","taoItems/assets/strategies","taoQtiPrint/lib/qrcode"],function($,_,async,helpers,itemRunner,assetStrategies,QRCode){"use str11ict";var testRenderer={createPage:function(type){return $("<section></section>").addClass(type);
},testPage:function(test,done){var $testPage=this.createPage("title"),$codeElt=$('<div class="qr-code">');new QRCode($codeElt[0],{text:helpers._url("render","PrintTest","taoBooklet",{uri:test.uri}),width:192,height:192,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H});$testPage.append("<h1>"+test.title+"</h1>").append($codeElt),done(null,$testPage)},sectionPage:function(section,pageNum,done){var $sectionPage=this.createPage("section");$sectionPage.append("<h2>"+section.title+"</h2>"),_.forEach(section.rubricBlock,function(rubricBlock){$sectionPage.append("<div>"+rubricBlock+"</div>")}),done(null,$sectionPage)},itemPage:function(item,uri,pageNum,assets,done){
var $itemContainer=this.createPage("item");itemRunner("qtiprint",item).on("error",function(err){done(err)}).on("render",function(){done(null,$itemContainer)}).assets([assetStrategies.taomedia,assetStrategies.external,{name:"packageAssetHandler",handle:function(url){for(var type in assets)if(assets[type][url.toString()])return assets[type][url.toString()]}}],{baseUrl:helpers._url("getFile","QtiCreator","taoQtiItem",{uri:uri,lang:"en-US"})+"&relPath="}).init().render($itemContainer)}},testRunnerFactory=function(testData,options){if(testData=testData||{},!testData||!testData.data||!testData.items)throw new Error("Invalid test data structure");var events={},testRunner={
$container:null,render:function(elt){var self=this,pageRenderers=[];return elt instanceof HTMLElement||elt instanceof $?(elt instanceof $?this.$container=elt:this.$container=$(elt),pageRenderers.push(_.bind(_.partial(testRenderer.testPage,testData.data),testRenderer)),_.forEach(testData.data.testParts,function(testPart){_.forEach(testPart.sections,function(section){pageRenderers.push(_.bind(_.partial(testRenderer.sectionPage,section,pageRenderers.length),testRenderer)),_.forEach(section.items,function(item){testData.items[item.href]&&testData.items[item.href].data&&pageRenderers.push(_.bind(_.partial(testRenderer.itemPage,testData.items[item.href].data,item.href,pageRenderers.length,testData.items[item.href].assets),testRenderer));
})})}),async.parallel(pageRenderers,function(err,$results){err&&self.trigger("error","An error occurred while rendering a page :  "+err),options&&options.rtl===!0&&self.$container.addClass("rtl").attr("dir","rtl"),self.$container.empty().append($results),self.trigger("render"),self.trigger("ready")}),this):self.trigger("error","A valid HTMLElement (or a jquery element) at least is required to render the test")},clear:function(){return this.$container.empty(),this.trigger("clear"),this},on:function(name,handler){return _.isString(name)&&_.isFunction(handler)&&(events[name]=events[name]||[],events[name].push(handler)),this},off:function(name){return _.isString(name)&&(events[name]=[]),
this},trigger:function(name,data){var self=this;return _.isString(name)&&_.isArray(events[name])&&_.forEach(events[name],function(event){event.call(self,data)}),this}};return testRunner};return testRunnerFactory}),define("taoBooklet/controller/printtest/render",["jquery","taoQtiPrint/runner/testRunner"],function($,testRunner){"use strict";var ready=function(){window.status="runner-ready"},showMessage=function(msg,type){type=type||"info",$("body").append('<div class="feedback-'+type+'">'+msg+"</div>")},printLayoutHacking=function($container){var previous,threshold=6e3,pages=2,blankPage=function($section){$section.before('<div class="breaker"></div>'),pages++},computePageFromHeight=function(height){
height>threshold&&(pages+=Math.ceil(height/threshold)-1)};$container.children("section").each(function(){var height,$section=$(this);$section.hasClass("section")&&(pages%2===0&&blankPage($section),computePageFromHeight($section.outerHeight()),previous="section",pages++),$section.hasClass("item")&&("item"!==previous&&(pages%2!==0&&blankPage($section),height=$section.outerHeight(),$section.nextUntil(".section",".item").each(function(){height+=$(this).outerHeight()}),computePageFromHeight(height),pages++),previous="item")})},renderController={start:function(testData,options){var timeout=setTimeout(function(){showMessage("Something went wrong...","error"),ready();
},45e3),$mainContainer=$("<main>");testRunner(testData,options).on("error",function(e){console.error(e),showMessage(e,"error"),ready()}).on("ready",function(){$("body").append($mainContainer),printLayoutHacking($mainContainer),clearTimeout(timeout),ready()}).render($mainContainer)}};return renderController});
