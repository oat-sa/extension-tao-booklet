...
Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))};return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoQtiItem/runner/provider/manager/picManager",["jquery","lodash","taoQtiItem/qtiItem/core/Element"],function($,_,Element){"use strict";var picManager={init:function(pic,item){return Element.isA(pic,"infoControl")&&(this._pic=pic),Element.isA(item,"assessmentItem")&&(this._item=item),this},getPic:function(){return this._pic},getItem:function(){return this._item},getSerial:function(){return this._pic&&this._pic.serial},getTypeIdentifier:function(){return this._pic&&this._pic.typeIdentifier;
},getDom:function(){if(!this._dom){var pic,tool,serial=this.getSerial();serial&&(pic=$('[data-serial="'+serial+'"]'),pic.length&&(tool=$('[data-pic-serial="'+serial+'"]'),tool.length||(tool=pic.children().first()),this._dom={pic:pic,tool:tool,button:tool.find(".sts-button"),broken:pic.is(":empty")}))}return this._dom},enable:function(){var dom=this.getDom();return dom&&(dom.button.removeClass("disabled"),dom.tool.find(".sts-button-disable-mask").remove(),this.disabled=!1,this.trigger("enable")),this},disable:function(){var button,dom=this.getDom();return dom&&(button=dom.button.addClass("disabled"),$('<div class="sts-button-disable-mask" style="position:absolute;z-index:10000000000000"></div>').appendTo(dom.tool).offset(button.offset()).width(button.outerWidth()).height(button.outerHeight()),
...
self._list=[],self._item&&_.forEach(self._item.getElements(),function(element){var manager;Element.isA(element,"infoControl")&&(manager=managerFactory(element,self._item),self._list.push(manager),self._map[element.serial]=manager,self._map[element.typeIdentifier]=manager)})),this._list},getPic:function(picId){return this.getList(),this._map[picId]},execute:function(picId,action){var pic=this.getPic(picId);if(pic&&pic[action])return pic[action].apply(pic,_.rest(arguments,2))},executeAll:function(action,filter){var cb,args=_.rest(arguments,2);return cb="function"==typeof filter?function(pic){filter.call(pic,pic)&&pic[action]&&pic[action].apply(pic,args)}:function(pic){
...
handle:function(url,data){if("string"==typeof data.baseUrl&&urlUtil.isRelative(url)&&/\.css$/.test(url.file))return urlUtil.build(prependToUrl(url,data.baseUrl,data.slashcat),{bust:Date.now()})}},external:{name:"external",handle:function(url){if(urlUtil.isAbsolute(url))return url.toString()}},base64:{name:"base64",handle:function(url){if(urlUtil.isBase64(url))return url.toString()}},taomedia:{name:"taomedia",handle:function(url,data){var baseUrl=data.baseUrl||"./";if("object"==typeof url&&"taomedia"===url.protocol||/^taomedia:\/\//.test(url.toString()))return baseUrl+url.toString()}}};return strategies}),define("taoQtiPrint/lib/qrcode",[],function(){function QR8bitByte(data){
...
null==this.dataCache&&(this.dataCache=QRCodeModel.createData(this.typeNumber,this.errorCorrectLevel,this.dataList)),this.mapData(this.dataCache,maskPattern)},setupPositionProbePattern:function(row,col){for(var r=-1;r<=7;r++)if(!(row+r<=-1||this.moduleCount<=row+r))for(var c=-1;c<=7;c++)col+c<=-1||this.moduleCount<=col+c||(0<=r&&r<=6&&(0==c||6==c)||0<=c&&c<=6&&(0==r||6==r)||2<=r&&r<=4&&2<=c&&c<=4?this.modules[row+r][col+c]=!0:this.modules[row+r][col+c]=!1)},getBestMaskPattern:function(){for(var minLostPoint=0,pattern=0,i=0;i<8;i++){this.makeImpl(!0,i);var lostPoint=QRUtil.getLostPoint(this);(0==i||minLostPoint>lostPoint)&&(minLostPoint=lostPoint,pattern=i)}return pattern;
},createMovieClip:function(target_mc,instance_name,depth){var qr_mc=target_mc.createEmptyMovieClip(instance_name,depth),cs=1;this.make();for(var row=0;row<this.modules.length;row++)for(var y=row*cs,col=0;col<this.modules[row].length;col++){var x=col*cs,dark=this.modules[row][col];dark&&(qr_mc.beginFill(0,100),qr_mc.moveTo(x,y),qr_mc.lineTo(x+cs,y),qr_mc.lineTo(x+cs,y+cs),qr_mc.lineTo(x,y+cs),qr_mc.endFill())}return qr_mc},setupTimingPattern:function(){for(var r=8;r<this.moduleCount-8;r++)null==this.modules[r][6]&&(this.modules[r][6]=r%2==0);for(var c=8;c<this.moduleCount-8;c++)null==this.modules[6][c]&&(this.modules[6][c]=c%2==0)},setupPositionAdjustPattern:function(){
for(var pos=QRUtil.getPatternPosition(this.typeNumber),i=0;i<pos.length;i++)for(var j=0;j<pos.length;j++){var row=pos[i],col=pos[j];if(null==this.modules[row][col])for(var r=-2;r<=2;r++)for(var c=-2;c<=2;c++)r==-2||2==r||c==-2||2==c||0==r&&0==c?this.modules[row+r][col+c]=!0:this.modules[row+r][col+c]=!1}},setupTypeNumber:function(test){for(var bits=QRUtil.getBCHTypeNumber(this.typeNumber),i=0;i<18;i++){var mod=!test&&1==(bits>>i&1);this.modules[Math.floor(i/3)][i%3+this.moduleCount-8-3]=mod}for(var i=0;i<18;i++){var mod=!test&&1==(bits>>i&1);this.modules[i%3+this.moduleCount-8-3][Math.floor(i/3)]=mod}},setupTypeInfo:function(test,maskPattern){for(var data=this.errorCorrectLevel<<3|maskPattern,bits=QRUtil.getBCHTypeInfo(data),i=0;i<15;i++){
var mod=!test&&1==(bits>>i&1);i<6?this.modules[i][8]=mod:i<8?this.modules[i+1][8]=mod:this.modules[this.moduleCount-15+i][8]=mod}for(var i=0;i<15;i++){var mod=!test&&1==(bits>>i&1);i<8?this.modules[8][this.moduleCount-i-1]=mod:i<9?this.modules[8][15-i-1+1]=mod:this.modules[8][15-i-1]=mod}this.modules[this.moduleCount-8][8]=!test},mapData:function(data,maskPattern){for(var inc=-1,row=this.moduleCount-1,bitIndex=7,byteIndex=0,col=this.moduleCount-1;col>0;col-=2)for(6==col&&col--;;){for(var c=0;c<2;c++)if(null==this.modules[row][col-c]){var dark=!1;byteIndex<data.length&&(dark=1==(data[byteIndex]>>>bitIndex&1));var mask=QRUtil.getMask(maskPattern,row,col-c);mask&&(dark=!dark),
this.modules[row][col-c]=dark,bitIndex--,bitIndex==-1&&(byteIndex++,bitIndex=7)}if(row+=inc,row<0||this.moduleCount<=row){row-=inc,inc=-inc;break}}}},QRCodeModel.PAD0=236,QRCodeModel.PAD1=17,QRCodeModel.createData=function(typeNumber,errorCorrectLevel,dataList){for(var rsBlocks=QRRSBlock.getRSBlocks(typeNumber,errorCorrectLevel),buffer=new QRBitBuffer,i=0;i<dataList.length;i++){var data=dataList[i];buffer.put(data.mode,4),buffer.put(data.getLength(),QRUtil.getLengthInBits(data.mode,typeNumber)),data.write(buffer)}for(var totalDataCount=0,i=0;i<rsBlocks.length;i++)totalDataCount+=rsBlocks[i].dataCount;if(buffer.getLengthInBits()>8*totalDataCount)throw new Error("code length overflow. ("+buffer.getLengthInBits()+">"+8*totalDataCount+")");
for(buffer.getLengthInBits()+4<=8*totalDataCount&&buffer.put(0,4);buffer.getLengthInBits()%8!=0;)buffer.putBit(!1);for(;;){if(buffer.getLengthInBits()>=8*totalDataCount)break;if(buffer.put(QRCodeModel.PAD0,8),buffer.getLengthInBits()>=8*totalDataCount)break;buffer.put(QRCodeModel.PAD1,8)}return QRCodeModel.createBytes(buffer,rsBlocks)},QRCodeModel.createBytes=function(buffer,rsBlocks){for(var offset=0,maxDcCount=0,maxEcCount=0,dcdata=new Array(rsBlocks.length),ecdata=new Array(rsBlocks.length),r=0;r<rsBlocks.length;r++){var dcCount=rsBlocks[r].dataCount,ecCount=rsBlocks[r].totalCount-dcCount;maxDcCount=Math.max(maxDcCount,dcCount),maxEcCount=Math.max(maxEcCount,ecCount),
dcdata[r]=new Array(dcCount);for(var i=0;i<dcdata[r].length;i++)dcdata[r][i]=255&buffer.buffer[i+offset];offset+=dcCount;var rsPoly=QRUtil.getErrorCorrectPolynomial(ecCount),rawPoly=new QRPolynomial(dcdata[r],rsPoly.getLength()-1),modPoly=rawPoly.mod(rsPoly);ecdata[r]=new Array(rsPoly.getLength()-1);for(var i=0;i<ecdata[r].length;i++){var modIndex=i+modPoly.getLength()-ecdata[r].length;ecdata[r][i]=modIndex>=0?modPoly.get(modIndex):0}}for(var totalCodeCount=0,i=0;i<rsBlocks.length;i++)totalCodeCount+=rsBlocks[i].totalCount;for(var data=new Array(totalCodeCount),index=0,i=0;i<maxDcCount;i++)for(var r=0;r<rsBlocks.length;r++)i<dcdata[r].length&&(data[index++]=dcdata[r][i]);
...
this._oQRCode.make(),this._el.title=sText,this._oDrawing.draw(this._oQRCode),this.makeImage()},QRCode.prototype.makeImage=function(){"function"==typeof this._oDrawing.makeImage&&(!this._android||this._android>=3)&&this._oDrawing.makeImage()},QRCode.prototype.clear=function(){this._oDrawing.clear()},QRCode.CorrectLevel=QRErrorCorrectLevel,QRCode}),define("tpl!taoQtiPrint/runner/tpl/pageTest",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,options,buffer="";return buffer+='\n    <div class="logo">\n        <img src="',(helper=helpers.logo)?stack1=helper.call(depth0,{
hash:{},data:data}):(helper=depth0&&depth0.logo,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" alt="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Logo",options):helperMissing.call(depth0,"__","Logo",options)))+'" />\n    </div>\n'}function program3(depth0,data){var stack1,helper,buffer="";return buffer+="\n    <h2>",(helper=helpers.subtitle)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.subtitle,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h2>\n";
}function program5(depth0,data){var stack1,helper,buffer="";return buffer+="\n    <h3>",(helper=helpers.date)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.date,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h3>\n"}function program7(depth0,data){return'\n    <div class="qr-code"></div>\n'}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing,self=this;return stack1=helpers.if.call(depth0,depth0&&depth0.logo,{
hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n    <h1>",(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h1>\n",stack1=helpers.if.call(depth0,depth0&&depth0.subtitle,{hash:{},inverse:self.noop,fn:self.program(3,program3,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n",stack1=helpers.if.call(depth0,depth0&&depth0.date,{hash:{},inverse:self.noop,fn:self.program(5,program5,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
buffer+="\n",stack1=helpers.if.call(depth0,depth0&&depth0.qrcode,{hash:{},inverse:self.noop,fn:self.program(7,program7,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n"})}),define("tpl!taoQtiPrint/runner/tpl/pageSection",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,buffer="";return buffer+="\n<div>",stack1=typeof depth0===functionType?depth0.apply(depth0):depth0,(stack1||0===stack1)&&(buffer+=stack1),buffer+="</div>\n"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;
return buffer+="<h2>",(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h2>\n",stack1=helpers.each.call(depth0,depth0&&depth0.rubricBlock,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n"})}),define("tpl!taoQtiPrint/runner/tpl/pageBlock",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};
var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<section class="grid-row ',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></section>\n'})}),define("taoQtiPrint/runner/testRenderers",["jquery","lodash","util/url","taoQtiPrint/runner/itemRunner","taoItems/assets/strategies","taoQtiPrint/lib/qrcode","tpl!taoQtiPrint/runner/tpl/pageTest","tpl!taoQtiPrint/runner/tpl/pageSection","tpl!taoQtiPrint/runner/tpl/pageBlock"],function($,_,urlHelper,itemRunner,assetStrategies,QRCode,pageTestTpl,pageSectionTpl){
"use strict";return{testPage:function($container,test,options,done){var coverPageOptions=options&&options.cover_page||{};$(pageTestTpl({title:coverPageOptions.title&&test.title,subtitle:coverPageOptions.description&&options.description,qrcode:coverPageOptions.qr_code,logo:coverPageOptions.logo&&options.logo,date:coverPageOptions.date&&options.date})).appendTo($container),coverPageOptions.qr_code&&new QRCode($(".qr-code",$container).get(0),{text:urlHelper.route("render","PrintTest","taoBooklet",{uri:test.uri}),width:192,height:192,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H}),done()},sectionPage:function($container,section,done){
$(pageSectionTpl({title:section.title,rubricBlock:section.rubricBlock})).appendTo($container),done()},itemPage:function($container,itemRef,itemState,uri,done){function itemDone(err){isDone||(isDone=!0,done(err))}var item=itemRef.data,assets=itemRef.assets,baseUrl=itemRef.baseUrl||urlHelper.route("getFile","QtiCreator","taoQtiItem",{uri:uri,lang:"en-US"})+"&relPath=",isDone=!1;itemRunner("qtiprint",item,{renderer:itemRef.renderer}).on("error",function(err){itemDone(err)}).on("init",function(){itemState&&_.isPlainObject(itemState)&&this.setState(itemState),this.render($container)}).on("ready",function(){itemDone()}).assets([{name:"base64package",handle:function(url){
var id=url.file,found=_.find(assets,function(assetsList){var asset=assetsList[id];return!(!asset||!urlHelper.isBase64(asset))});if(found)return found[id]}},assetStrategies.taomedia,assetStrategies.external,assetStrategies.baseUrl],{baseUrl:baseUrl}).init()}}}),define("taoQtiPrint/runner/testRunner",["jquery","lodash","async","core/eventifier","taoQtiPrint/runner/testRenderers","tpl!taoQtiPrint/runner/tpl/pageBlock"],function($,_,asyncLib,eventifier,testRenderers,pageBlockTpl){"use strict";var testRunnerFactory=function(testDefinition,options){function createPage(type){var $pageBlock=$(pageBlockTpl({type:type}));return $container.append($pageBlock),$pageBlock}
var $container,layoutOptions=options&&options.layout||{};if(!testDefinition||!testDefinition.data||!testDefinition.items)throw new Error("Invalid test data structure");return eventifier({render:function(elt){var self=this,pageRenderers=[],testData=testDefinition.data||{},items=testDefinition.items||{},states=testDefinition.states||{};return elt instanceof HTMLElement||elt instanceof $?($container=elt instanceof $?elt:$(elt),$container.empty(),options&&options.rtl===!0&&$container.addClass("rtl").attr("dir","rtl"),layoutOptions.cover_page&&pageRenderers.push(_.partial(testRenderers.testPage,createPage("title"),testData,options)),_.forEach(testData.testParts,function(testPart){
_.forEach(testPart.sections,function(section){pageRenderers.push(_.partial(testRenderers.sectionPage,createPage("section"),section)),_.forEach(section.items,function(item){var itemRef=items[item.href],itemState=states[item.href];itemRef&&itemRef.data&&(itemRef.renderer=options.regular?"results":"booklet",pageRenderers.push(_.partial(testRenderers.itemPage,createPage("item"),itemRef,itemState,item.href)))})})}),asyncLib.parallel(pageRenderers,function(err){err&&self.trigger("error","An error occurred while rendering a page :  "+err),self.trigger("render"),self.trigger("ready")}),this):self.trigger("error","A valid HTMLElement (or a jquery element) at least is required to render the test");
},clear:function(){return $container.empty(),this.trigger("clear"),this}})};return testRunnerFactory}),define("tpl!taoBooklet/tpl/print",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<main class="tao-scope ',(helper=helpers.cls)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.cls,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></main>\n';
})}),define("taoBooklet/controller/PrintTest/render",["jquery","lodash","core/logger","taoQtiPrint/runner/testRunner","tpl!taoBooklet/tpl/print"],function($,_,loggerFactory,testRunner,printTpl){"use strict";function showMessage(msg,type){type=type||"info",$("body").append('<div class="feedback-'+type+'">'+msg+"</div>")}function printLayoutHacking($container){var previous,threshold=6e3,pages=2,blankPage=function($section){$section.before('<div class="breaker"></div>'),pages++},computePageFromHeight=function(height){height>threshold&&(pages+=Math.ceil(height/threshold)-1)};$container.children("section").each(function(){var height,$section=$(this);$section.hasClass("section")&&(pages%2===0&&blankPage($section),
computePageFromHeight($section.outerHeight()),previous="section",pages++),$section.hasClass("item")&&("item"!==previous&&(pages%2!==0&&blankPage($section),height=$section.outerHeight(),$section.nextUntil(".section",".item").each(function(){height+=$(this).outerHeight()}),computePageFromHeight(height),pages++),previous="item")})}var logger=loggerFactory("printTest/render");return{start:function(testData,options){function ready(){window.status="runner-ready"}var layoutOptions=options&&options.layout||{},layoutClasses={one_page_item:"one-item-per-page",one_page_section:"one-section-per-page"},timeout=setTimeout(function(){showMessage("Something went wrong...","error"),
ready()},45e3),$mainContainer=$(printTpl({cls:_.reduce(layoutOptions,function(list,value,name){return value&&layoutClasses[name]&&list.push(layoutClasses[name]),list},[]).join(" ")}));$("body").append($mainContainer),testRunner(testData,options).on("error",function(e){logger.error(e),showMessage(e,"error"),ready()}).on("ready",function(){layoutOptions.add_blank_pages&&printLayoutHacking($mainContainer),clearTimeout(timeout),ready()}).render($mainContainer)}}}),define("taoBooklet/component/taskQueue",["jquery","lodash","util/url","ui/taskQueue/table"],function($,_,urlHelper,taskQueueTableFactory){"use strict";function bookletTaskQueue(queueId,$container,config){
var taskQueue=taskQueueTableFactory(_.merge({context:queueId},config||{},defaults));return taskQueue.init().render($container),taskQueue}var defaults={rows:10,dataUrl:urlHelper.route("getTasks","TaskQueueData","taoBooklet"),statusUrl:urlHelper.route("getStatus","TaskQueueData","taoBooklet"),removeUrl:urlHelper.route("archiveTask","TaskQueueData","taoBooklet")};return bookletTaskQueue}),define("taoBooklet/controller/Results/printWizard",["jquery","lodash","ui/hider","util/url","report","taoBooklet/component/taskQueue","jquery.fileDownload"],function($,_,hider,urlHelper,report,taskQueueTableFactory){"use strict";return{start:function(){function switchContainer(purpose){
hider.hide($containers),hider.show($containers.filter('[data-purpose="'+purpose+'"]'))}function refreshTree(){$(".tree").trigger("refresh.taotree",[{uri:$header.data("select-node")}])}function displayReport(response){switchContainer("report"),$reportContainer.append(response),hider.toggle($("#fold",$reportContainer),response.nested),$('#fold > input[type="checkbox"]',$reportContainer).on("click",function(){report.fold()}),$("#import-continue",$reportContainer).on("click",refreshTree)}var $header=$("header.section-header"),$formContainer=$(".print-form"),$reportContainer=$(".print-report"),$form=$("form",$formContainer),$submitter=$(".form-submitter",$form),$sent=$(":input[name='"+$form.attr("name")+"_sent']",$form),asyncQueue=$header.data("async-queue"),$containers=$(".main-container");
switchContainer("form"),$submitter.off("click").on("click",function(e){var params={},instances=[],classes=[];e.preventDefault(),parseInt($sent.val(),10)&&(_.forEach($form.serializeArray(),function(param){0===param.name.indexOf("instances_")?instances.push(param.value):0===param.name.indexOf("classes_")?classes.push(param.value):params[param.name]=param.value}),params.instances=encodeURIComponent(JSON.stringify(instances)),params.classes=encodeURIComponent(JSON.stringify(classes)),asyncQueue?$.ajax({url:$form.attr("action"),data:params,type:"POST",dataType:"text"}).done(displayReport):$.fileDownload($form.attr("action"),{httpMethod:"POST",data:params,failCallback:displayReport,
