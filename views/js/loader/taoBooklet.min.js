...
"continue",function(){refreshTree()}).on("error",function(err){feedback().error(err),$("#booklet-new a").click()}).render($oldSubmitter.closest(".form-toolbar")),$oldSubmitter.replaceWith(taskCreationButton.getElement().css({float:"right"}))}}}),define("tpl!taoBooklet/tpl/print",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="";return buffer+='<main class="tao-scope ',buffer+=(0,this.escapeExpression)((helper=helpers.cls)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(
helper=depth0&&depth0.cls)?helper.call(depth0,{hash:{},data:data}):helper)+'"></main>'})}),define("taoBooklet/controller/PrintTest/render",["jquery","lodash","core/logger","taoQtiPrint/runner/testRunner","tpl!taoBooklet/tpl/print"],function($,_,loggerFactory,testRunner,printTpl){"use strict";var logger=loggerFactory("printTest/render");function showMessage(msg,type){type=type||"info",$("body").append('<div class="feedback-'+type+'">'+msg+"</div>")}return{start:function(config){var testData=config.testData,options=config.options,layoutOptions=options&&options.layout||{},layoutClasses={one_page_item:"one-item-per-page",one_page_section:"one-section-per-page"},
