...
self._itemData=itemData,self._item=item;try{self._renderer.load(function(){self._item.setRenderer(this),done()},this.getLoadedClasses())}catch(e){self.trigger("error","Unable to render the item : "+e)}})})},render:function(elt,done){var self=this;if(this._item){try{elt.innerHTML=this._item.render({})}catch(e){logger.trace(e),elt.innerHTML=missingSupport(this._itemData,e),self.trigger("warning","Error in template rendering : "+e)}try{Promise.race([Promise.all(this._item.postRender()),new Promise(function(resolve,reject){_.delay(reject,timeout,new Error("Post rendering ran out of time in item "+self._itemData.serial))})]).then(function(){return self.trigger("listpic",picManager.collection(self._item)),
userModules.load().then(done)}).catch(function(err){self.trigger("error","Error in post rendering : "+err instanceof Error?err.message:err),done()})}catch(e){self.trigger("error","Error in post rendering : "+e),done()}}else done()},clear:function(elt,done){done()},getState:function(){return{}},setState:function(state){this._item&&state&&(_.forEach(this._item.getInteractions(),function(interaction){var id=interaction.attr("responseIdentifier");id&&state[id]&&interaction.setState(state[id])}),state.pic&&_.forEach(this._item.getElements(),function(element){Element.isA(element,"infoControl")&&state.pic[element.attr("id")]&&element.setState(state.pic[element.attr("id")]);
