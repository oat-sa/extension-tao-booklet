define("taoBooklet/controller/routes",[],function(){"use strict";return{Booklet:{actions:{editBooklet:"controller/Booklet/editBooklet",wizard:"controller/Booklet/wizard",preview:"controller/Booklet/preview"}},Results:{actions:{printWizard:"controller/Results/printWizard"}}}}),define("taoBooklet/controller/Booklet/editBooklet",["jquery","lodash","module","util/url"],function($,_,module,urlHelper){"use strict";return{start:function(){var $downloader=$("<iframe/>").hide();$("form").append($downloader),$(".btn-download").on("click",function(e){e.preventDefault();var $form=$(e.target).closest("form");$downloader.attr("src",urlHelper.route("download","Booklet","taoBooklet",{
uri:$form.find('input[name="uri"]').val(),classUri:$form.find('input[name="classUri"]').val(),id:$form.find('input[name="id"]').val()}))})}}}),define("taoBooklet/controller/Booklet/preview",["jquery"],function($){"use strict";return{start:function(){var $frame=$(".preview-test"),$container=$frame.closest(".content-container");$frame.css("min-height",$container.height()-50+"px")}}}),define("taoBooklet/controller/Booklet/wizard",["jquery"],function($){"use strict";return{start:function(){$(".main-container ").find("form")}}}),define("taoItems/assets/manager",["lodash","util/url"],function(_,urlUtil){"use strict";var checkStrategy=function(strategy){if(!_.isPlainObject(strategy)||!_.isFunction(strategy.handle)||!_.isString(strategy.name))throw new TypeError("An asset resolution strategy is an object with a handle method and a name");
},assetManagerFactory=function(strategies,data,options){var cache={};strategies=_.isArray(strategies)?strategies:[strategies],data=data||{},options=_.defaults(options||{},{parseUrl:!0});var assetManager={_strategies:[],addStrategy:function(strategy){checkStrategy(strategy),this._strategies.push(strategy)},prependStrategy:function(strategy){checkStrategy(strategy),this._strategies.unshift(strategy)},setStrategies:function(newStrategies){var self=this;this._strategies=[],_.forEach(newStrategies,function(strategy){_.isPlainObject(strategy)?assetManager.addStrategy(strategy):_.isFunction(strategy)&&self.addStrategy({name:"strategy_"+(self._strategies.length+1),handle:strategy
...
if(data.choices){if(Element.isA(interaction,"matchInteraction"))for(var set=0;set<2;set++){if(!data.choices[set])throw"missing match set #"+set;var matchSet=data.choices[set];for(var serial in matchSet){var choice=this.buildElement(matchSet[serial]);choice&&interaction.addChoice(choice,set)}}else for(var serial in data.choices){var choice=this.buildElement(data.choices[serial]);choice&&interaction.addChoice(choice)}if(Element.isA(interaction,"graphicGapMatchInteraction")&&data.gapImgs)for(var serial in data.gapImgs){var gapImg=this.buildElement(data.gapImgs[serial]);gapImg&&interaction.addGapImg(gapImg)}}},loadChoiceData:function(choice,data){Element.isA(choice,"textVariableChoice")?choice.val(data.text):Element.isA(choice,"gapImg")||(Element.isA(choice,"gapText")?choice.body()||choice.body(data.text):Element.isA(choice,"containerChoice"));
...
var options=_.isPlainObject(arguments[0])?arguments[0]:{};Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))};return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoQtiItem/runner/provider/manager/picManager",["jquery","lodash","taoQtiItem/qtiItem/core/Element"],function($,_,Element){"use strict";var picManager={init:function(pic,item){return Element.isA(pic,"infoControl")&&(this._pic=pic),Element.isA(item,"assessmentItem")&&(this._item=item),this},getPic:function(){return this._pic},getItem:function(){return this._item},getSerial:function(){return this._pic&&this._pic.serial;
},getTypeIdentifier:function(){return this._pic&&this._pic.typeIdentifier},getDom:function(){if(!this._dom){var pic,tool,serial=this.getSerial();serial&&(pic=$('[data-serial="'+serial+'"]'),pic.length&&(tool=$('[data-pic-serial="'+serial+'"]'),tool.length||(tool=pic.children().first()),this._dom={pic:pic,tool:tool,button:tool.find(".sts-button"),broken:pic.is(":empty")}))}return this._dom},enable:function(){var dom=this.getDom();return dom&&(dom.button.removeClass("disabled"),dom.tool.find(".sts-button-disable-mask").remove(),this.disabled=!1,this.trigger("enable")),this},disable:function(){var button,dom=this.getDom();return dom&&(button=dom.button.addClass("disabled"),
$('<div class="sts-button-disable-mask" style="position:absolute;z-index:10000000000000"></div>').appendTo(dom.tool).offset(button.offset()).width(button.outerWidth()).height(button.outerHeight()),dom.tool.find(".sts-container, .sts-menu-container").addClass("sts-hidden-container"),this.disabled=!0,this.trigger("disable")),this},show:function(){var dom=this.getDom();return dom&&(dom.tool.show(),this.trigger("show")),this},hide:function(){var dom=this.getDom();return dom&&(dom.tool.hide(),this.trigger("hide")),this},trigger:function(eventName){var dom=this.getDom(),args=_.rest(arguments,1);return args.unshift(this),dom&&(dom.tool.trigger(eventName,args),dom.broken&&dom.pic.trigger(eventName,args)),
this}},picManagerCollection={init:function(item){return Element.isA(item,"assessmentItem")&&(this._item=item),this},getList:function(force){var self=this;return!force&&self._list||(self._map={},self._list=[],self._item&&_.forEach(self._item.getElements(),function(element){var manager;Element.isA(element,"infoControl")&&(manager=managerFactory(element,self._item),self._list.push(manager),self._map[element.serial]=manager,self._map[element.typeIdentifier]=manager)})),this._list},getPic:function(picId){return this.getList(),this._map[picId]},execute:function(picId,action){var pic=this.getPic(picId);if(pic&&pic[action])return pic[action].apply(pic,_.rest(arguments,2));
},executeAll:function(action,filter){var cb,args=_.rest(arguments,2);return cb="function"==typeof filter?function(pic){filter.call(pic,pic)&&pic[action]&&pic[action].apply(pic,args)}:function(pic){pic[action]&&pic[action].apply(pic,args)},this.each(cb)},each:function(cb){return _.forEach(this.getList(),cb),this},enablePic:function(picId){return this.execute(picId,"enable"),this},disablePic:function(picId){return this.execute(picId,"disable"),this},showPic:function(picId){return this.execute(picId,"show"),this},hidePic:function(picId){return this.execute(picId,"hide"),this},enableAll:function(filter){return this.executeAll("enable",filter),this},disableAll:function(filter){
return this.executeAll("disable",filter),this},showAll:function(filter){return this.executeAll("show",filter),this},hideAll:function(filter){return this.executeAll("hide",filter),this}},managerFactory=function(pic,item){var manager=_.clone(picManager,!0);return manager.init(pic,item)},collectionFactory=function(item){var collection=_.clone(picManagerCollection,!0);return collection.init(item)};return{collection:collectionFactory,manager:managerFactory}}),define("taoQtiItem/runner/provider/manager/userModules",["lodash","module","core/promise"],function(_,module,Promise){"use strict";return{load:function(userModules){var config=module.config();return userModules&&_.isArray(userModules)||(userModules=config&&config.userModules&&_.isArray(config.userModules)?config.userModules:[]),
new Promise(function(resolve,reject){require(userModules,function(){_.forEach(arguments,function(dependency){dependency&&_.isFunction(dependency.exec)&&dependency.exec()}),resolve()},function(err){reject(err.message)})})}}}),define("tpl!taoQtiPrint/qtiPrintRenderer/tpl/notYetSupported",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\r\n    <div class="additional">\r\n        <div class="title">Additional information:</div>\r\n        <p class="info">',(helper=helpers.additional)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.additional,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+="</p>\r\n    </div>\r\n"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",self=this;return buffer+='<article class="render-error feedback-error">\r\n    <span class="icon-error"></span>\r\n    <p class="message">',(helper=helpers.message)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.message,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),
buffer+="</p>\r\n",stack1=helpers.if.call(depth0,depth0&&depth0.additional,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n</article>\r\n"})}),define("taoQtiPrint/runner/provider/qtiprint",["lodash","i18n","context","core/logger","core/promise","taoQtiItem/qtiItem/core/Loader","taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiRunner/core/Renderer","taoQtiItem/runner/provider/manager/picManager","taoQtiItem/runner/provider/manager/userModules","tpl!taoQtiPrint/qtiPrintRenderer/tpl/notYetSupported"],function(_,__,context,loggerFactory,Promise,QtiLoader,Element,Renderer,picManager,userModules,notYetSupportedTpl){
"use strict";function getQtiRenderer(rendererType){return QtiRendererPromise||(QtiRendererPromise=new Promise(function(resolve){require(rendererModules[rendererType],function(config){resolve(Renderer.build(config.locations,config.name,config.options))})}).catch(function(err){logger.error(err)})),QtiRendererPromise}function missingSupport(item,additional){var label=item.identifier;return item.attributes&&item.attributes.label&&(label=item.attributes.label),notYetSupportedTpl({message:__("The item &laquo;%s&raquo; contains an interaction that is not yet supported for printing purpose",label),additional:additional})}var timeout=1e3*(context.timeout>0?context.timeout+1:30),logger=loggerFactory("runner/provider/qtiPrint"),rendererModules={
results:["taoQtiPrint/qtiCommonRenderer/renderers/config","css!taoQtiPrintCss/themes/items/default/theme.css"],booklet:["taoQtiPrint/qtiPrintRenderer/renderers/config","css!taoQtiPrintCss/qti.css"],bubbleSheet:["taoQtiPrint/qtiPrintRenderer/renderers/config","css!taoQtiPrintCss/themes/items/bubble-sheet/theme.css"]},QtiRendererPromise=null;return{init:function(itemData,done){var self=this;getQtiRenderer(self.options.renderer).then(function(QtiRenderer){var rendererOptions={assetManager:self.assetManager};self.options.themes&&(rendererOptions.themes=self.options.themes),self._loader=new QtiLoader,self._renderer=new QtiRenderer(rendererOptions),self._loader.loadItemData(itemData,function(item){
if(!item)return self.trigger("error","Unable to load item from the given data.");self._itemData=itemData,self._item=item;try{self._renderer.load(function(){self._item.setRenderer(this),done()},this.getLoadedClasses())}catch(e){self.trigger("error","Unable to render the item : "+e)}})})},render:function(elt,done){var self=this;if(this._item){try{this.options.showResponseIdentifier&&this._item.addClass("show-response-identifier"),elt.innerHTML=this._item.render({})}catch(e){logger.trace(e),elt.innerHTML=missingSupport(this._itemData,e),self.trigger("warning","Error in template rendering : "+e)}try{Promise.race([Promise.all(this._item.postRender()),new Promise(function(resolve,reject){
_.delay(reject,timeout,new Error("Post rendering ran out of time in item "+self._itemData.serial))})]).then(function(){return self.trigger("listpic",picManager.collection(self._item)),userModules.load().then(done)}).catch(function(err){self.trigger("error","Error in post rendering : "+err instanceof Error?err.message:err),done()})}catch(e){self.trigger("error","Error in post rendering : "+e),done()}}else done()},clear:function(elt,done){done()},getState:function(){return{}},setState:function(state){this._item&&state&&(_.forEach(this._item.getInteractions(),function(interaction){var id=interaction.attr("responseIdentifier");id&&state[id]&&interaction.setState(state[id]);
}),state.pic&&_.forEach(this._item.getElements(),function(element){Element.isA(element,"infoControl")&&state.pic[element.attr("id")]&&element.setState(state.pic[element.attr("id")])}))},getResponses:function(){return[]}}}),define("taoQtiPrint/runner/itemRunner",["taoItems/runner/api/itemRunner","taoQtiPrint/runner/provider/qtiprint"],function(itemRunner,qtiRuntimeProvider){"use strict";return itemRunner.register("qtiprint",qtiRuntimeProvider),itemRunner}),define("taoItems/assets/strategies",["util/url","lodash"],function(urlUtil,_){"use strict";var prependToUrl=function(url,base,slashcat){return slashcat===!0?base.replace(/\/$/,"")+"/"+url.directory.replace(/^\.\//,"").replace(/^\//,"")+encodeURIComponent(url.file.replace(/^\.\//,"").replace(/^\//,"")):base+url.directory.replace(/^\.?\//,"")+encodeURIComponent(url.file.replace(/^\.?\//,""));
},strategies={packedUrl:{name:"packedUrl",handle:function(url,data){var type;if(!_.isUndefined(url.source)&&!_.isUndefined(data.assets)&&(type=_.findKey(data.assets,url.source),type&&urlUtil.isAbsolute(data.assets[type][url.source])))return data.assets[type][url.source]}},baseUrl:{name:"baseUrl",handle:function(url,data){if("string"==typeof data.baseUrl&&urlUtil.isRelative(url))return prependToUrl(url,data.baseUrl,data.slashcat)}},itemCssNoCache:{name:"itemCssNoCache",handle:function(url,data){if("string"==typeof data.baseUrl&&urlUtil.isRelative(url)&&/\.css$/.test(url.file))return urlUtil.build(prependToUrl(url,data.baseUrl,data.slashcat),{bust:Date.now()})}
},external:{name:"external",handle:function(url){if(urlUtil.isAbsolute(url))return url.toString()}},base64:{name:"base64",handle:function(url){if(urlUtil.isBase64(url))return url.toString()}},taomedia:{name:"taomedia",handle:function(url,data){var baseUrl=data.baseUrl||"./";if("object"==typeof url&&"taomedia"===url.protocol||/^taomedia:\/\//.test(url.toString()))return baseUrl+url.toString()}}};return strategies}),define("taoQtiPrint/lib/qrcode",[],function(){function QR8bitByte(data){this.mode=QRMode.MODE_8BIT_BYTE,this.data=data,this.parsedData=[];for(var i=0,l=this.data.length;i<l;i++){var byteArray=[],code=this.data.charCodeAt(i);code>65536?(byteArray[0]=240|(1835008&code)>>>18,
byteArray[1]=128|(258048&code)>>>12,byteArray[2]=128|(4032&code)>>>6,byteArray[3]=128|63&code):code>2048?(byteArray[0]=224|(61440&code)>>>12,byteArray[1]=128|(4032&code)>>>6,byteArray[2]=128|63&code):code>128?(byteArray[0]=192|(1984&code)>>>6,byteArray[1]=128|63&code):byteArray[0]=code,this.parsedData.push(byteArray)}this.parsedData=Array.prototype.concat.apply([],this.parsedData),this.parsedData.length!=this.data.length&&(this.parsedData.unshift(191),this.parsedData.unshift(187),this.parsedData.unshift(239))}function QRCodeModel(typeNumber,errorCorrectLevel){this.typeNumber=typeNumber,this.errorCorrectLevel=errorCorrectLevel,this.modules=null,this.moduleCount=0,
this.dataCache=null,this.dataList=[]}function QRPolynomial(num,shift){if(void 0==num.length)throw new Error(num.length+"/"+shift);for(var offset=0;offset<num.length&&0==num[offset];)offset++;this.num=new Array(num.length-offset+shift);for(var i=0;i<num.length-offset;i++)this.num[i]=num[i+offset]}function QRRSBlock(totalCount,dataCount){this.totalCount=totalCount,this.dataCount=dataCount}function QRBitBuffer(){this.buffer=[],this.length=0}function _isSupportCanvas(){return"undefined"!=typeof CanvasRenderingContext2D}function _getAndroid(){var android=!1,sAgent=navigator.userAgent;if(/android/i.test(sAgent)){android=!0;var aMat=sAgent.toString().match(/android ([0-9]\.[0-9])/i);
aMat&&aMat[1]&&(android=parseFloat(aMat[1]))}return android}function _getTypeNumber(sText,nCorrectLevel){for(var nType=1,length=_getUTF8Length(sText),i=0,len=QRCodeLimitLength.length;i<=len;i++){var nLimit=0;switch(nCorrectLevel){case QRErrorCorrectLevel.L:nLimit=QRCodeLimitLength[i][0];break;case QRErrorCorrectLevel.M:nLimit=QRCodeLimitLength[i][1];break;case QRErrorCorrectLevel.Q:nLimit=QRCodeLimitLength[i][2];break;case QRErrorCorrectLevel.H:nLimit=QRCodeLimitLength[i][3]}if(length<=nLimit)break;nType++}if(nType>QRCodeLimitLength.length)throw new Error("Too long data");return nType}function _getUTF8Length(sText){var replacedText=encodeURI(sText).toString().replace(/\%[0-9a-fA-F]{2}/g,"a");
return replacedText.length+(replacedText.length!=sText?3:0)}var QRCode;QR8bitByte.prototype={getLength:function(buffer){return this.parsedData.length},write:function(buffer){for(var i=0,l=this.parsedData.length;i<l;i++)buffer.put(this.parsedData[i],8)}},QRCodeModel.prototype={addData:function(data){var newData=new QR8bitByte(data);this.dataList.push(newData),this.dataCache=null},isDark:function(row,col){if(row<0||this.moduleCount<=row||col<0||this.moduleCount<=col)throw new Error(row+","+col);return this.modules[row][col]},getModuleCount:function(){return this.moduleCount},make:function(){this.makeImpl(!1,this.getBestMaskPattern())},makeImpl:function(test,maskPattern){
this.moduleCount=4*this.typeNumber+17,this.modules=new Array(this.moduleCount);for(var row=0;row<this.moduleCount;row++){this.modules[row]=new Array(this.moduleCount);for(var col=0;col<this.moduleCount;col++)this.modules[row][col]=null}this.setupPositionProbePattern(0,0),this.setupPositionProbePattern(this.moduleCount-7,0),this.setupPositionProbePattern(0,this.moduleCount-7),this.setupPositionAdjustPattern(),this.setupTimingPattern(),this.setupTypeInfo(test,maskPattern),this.typeNumber>=7&&this.setupTypeNumber(test),null==this.dataCache&&(this.dataCache=QRCodeModel.createData(this.typeNumber,this.errorCorrectLevel,this.dataList)),this.mapData(this.dataCache,maskPattern);
},setupPositionProbePattern:function(row,col){for(var r=-1;r<=7;r++)if(!(row+r<=-1||this.moduleCount<=row+r))for(var c=-1;c<=7;c++)col+c<=-1||this.moduleCount<=col+c||(0<=r&&r<=6&&(0==c||6==c)||0<=c&&c<=6&&(0==r||6==r)||2<=r&&r<=4&&2<=c&&c<=4?this.modules[row+r][col+c]=!0:this.modules[row+r][col+c]=!1)},getBestMaskPattern:function(){for(var minLostPoint=0,pattern=0,i=0;i<8;i++){this.makeImpl(!0,i);var lostPoint=QRUtil.getLostPoint(this);(0==i||minLostPoint>lostPoint)&&(minLostPoint=lostPoint,pattern=i)}return pattern},createMovieClip:function(target_mc,instance_name,depth){var qr_mc=target_mc.createEmptyMovieClip(instance_name,depth),cs=1;this.make();for(var row=0;row<this.modules.length;row++)for(var y=row*cs,col=0;col<this.modules[row].length;col++){
var x=col*cs,dark=this.modules[row][col];dark&&(qr_mc.beginFill(0,100),qr_mc.moveTo(x,y),qr_mc.lineTo(x+cs,y),qr_mc.lineTo(x+cs,y+cs),qr_mc.lineTo(x,y+cs),qr_mc.endFill())}return qr_mc},setupTimingPattern:function(){for(var r=8;r<this.moduleCount-8;r++)null==this.modules[r][6]&&(this.modules[r][6]=r%2==0);for(var c=8;c<this.moduleCount-8;c++)null==this.modules[6][c]&&(this.modules[6][c]=c%2==0)},setupPositionAdjustPattern:function(){for(var pos=QRUtil.getPatternPosition(this.typeNumber),i=0;i<pos.length;i++)for(var j=0;j<pos.length;j++){var row=pos[i],col=pos[j];if(null==this.modules[row][col])for(var r=-2;r<=2;r++)for(var c=-2;c<=2;c++)r==-2||2==r||c==-2||2==c||0==r&&0==c?this.modules[row+r][col+c]=!0:this.modules[row+r][col+c]=!1;
}},setupTypeNumber:function(test){for(var bits=QRUtil.getBCHTypeNumber(this.typeNumber),i=0;i<18;i++){var mod=!test&&1==(bits>>i&1);this.modules[Math.floor(i/3)][i%3+this.moduleCount-8-3]=mod}for(var i=0;i<18;i++){var mod=!test&&1==(bits>>i&1);this.modules[i%3+this.moduleCount-8-3][Math.floor(i/3)]=mod}},setupTypeInfo:function(test,maskPattern){for(var data=this.errorCorrectLevel<<3|maskPattern,bits=QRUtil.getBCHTypeInfo(data),i=0;i<15;i++){var mod=!test&&1==(bits>>i&1);i<6?this.modules[i][8]=mod:i<8?this.modules[i+1][8]=mod:this.modules[this.moduleCount-15+i][8]=mod}for(var i=0;i<15;i++){var mod=!test&&1==(bits>>i&1);i<8?this.modules[8][this.moduleCount-i-1]=mod:i<9?this.modules[8][15-i-1+1]=mod:this.modules[8][15-i-1]=mod;
}this.modules[this.moduleCount-8][8]=!test},mapData:function(data,maskPattern){for(var inc=-1,row=this.moduleCount-1,bitIndex=7,byteIndex=0,col=this.moduleCount-1;col>0;col-=2)for(6==col&&col--;;){for(var c=0;c<2;c++)if(null==this.modules[row][col-c]){var dark=!1;byteIndex<data.length&&(dark=1==(data[byteIndex]>>>bitIndex&1));var mask=QRUtil.getMask(maskPattern,row,col-c);mask&&(dark=!dark),this.modules[row][col-c]=dark,bitIndex--,bitIndex==-1&&(byteIndex++,bitIndex=7)}if(row+=inc,row<0||this.moduleCount<=row){row-=inc,inc=-inc;break}}}},QRCodeModel.PAD0=236,QRCodeModel.PAD1=17,QRCodeModel.createData=function(typeNumber,errorCorrectLevel,dataList){for(var rsBlocks=QRRSBlock.getRSBlocks(typeNumber,errorCorrectLevel),buffer=new QRBitBuffer,i=0;i<dataList.length;i++){
var data=dataList[i];buffer.put(data.mode,4),buffer.put(data.getLength(),QRUtil.getLengthInBits(data.mode,typeNumber)),data.write(buffer)}for(var totalDataCount=0,i=0;i<rsBlocks.length;i++)totalDataCount+=rsBlocks[i].dataCount;if(buffer.getLengthInBits()>8*totalDataCount)throw new Error("code length overflow. ("+buffer.getLengthInBits()+">"+8*totalDataCount+")");for(buffer.getLengthInBits()+4<=8*totalDataCount&&buffer.put(0,4);buffer.getLengthInBits()%8!=0;)buffer.putBit(!1);for(;;){if(buffer.getLengthInBits()>=8*totalDataCount)break;if(buffer.put(QRCodeModel.PAD0,8),buffer.getLengthInBits()>=8*totalDataCount)break;buffer.put(QRCodeModel.PAD1,8)}return QRCodeModel.createBytes(buffer,rsBlocks);
...
if(1<=type&&type<10)switch(mode){case QRMode.MODE_NUMBER:return 10;case QRMode.MODE_ALPHA_NUM:return 9;case QRMode.MODE_8BIT_BYTE:return 8;case QRMode.MODE_KANJI:return 8;default:throw new Error("mode:"+mode)}else if(type<27)switch(mode){case QRMode.MODE_NUMBER:return 12;case QRMode.MODE_ALPHA_NUM:return 11;case QRMode.MODE_8BIT_BYTE:return 16;case QRMode.MODE_KANJI:return 10;default:throw new Error("mode:"+mode)}else{if(!(type<41))throw new Error("type:"+type);switch(mode){case QRMode.MODE_NUMBER:return 14;case QRMode.MODE_ALPHA_NUM:return 13;case QRMode.MODE_8BIT_BYTE:return 16;case QRMode.MODE_KANJI:return 12;default:throw new Error("mode:"+mode)}}},getLostPoint:function(qrCode){
...
for(var col=0;col<moduleCount;col++)for(var row=0;row<moduleCount-6;row++)qrCode.isDark(row,col)&&!qrCode.isDark(row+1,col)&&qrCode.isDark(row+2,col)&&qrCode.isDark(row+3,col)&&qrCode.isDark(row+4,col)&&!qrCode.isDark(row+5,col)&&qrCode.isDark(row+6,col)&&(lostPoint+=40);for(var darkCount=0,col=0;col<moduleCount;col++)for(var row=0;row<moduleCount;row++)qrCode.isDark(row,col)&&darkCount++;var ratio=Math.abs(100*darkCount/moduleCount/moduleCount-50)/5;return lostPoint+=10*ratio}},QRMath={glog:function(n){if(n<1)throw new Error("glog("+n+")");return QRMath.LOG_TABLE[n]},gexp:function(n){for(;n<0;)n+=255;for(;n>=256;)n-=255;return QRMath.EXP_TABLE[n]},EXP_TABLE:new Array(256),
LOG_TABLE:new Array(256)},i=0;i<8;i++)QRMath.EXP_TABLE[i]=1<<i;for(var i=8;i<256;i++)QRMath.EXP_TABLE[i]=QRMath.EXP_TABLE[i-4]^QRMath.EXP_TABLE[i-5]^QRMath.EXP_TABLE[i-6]^QRMath.EXP_TABLE[i-8];for(var i=0;i<255;i++)QRMath.LOG_TABLE[QRMath.EXP_TABLE[i]]=i;QRPolynomial.prototype={get:function(index){return this.num[index]},getLength:function(){return this.num.length},multiply:function(e){for(var num=new Array(this.getLength()+e.getLength()-1),i=0;i<this.getLength();i++)for(var j=0;j<e.getLength();j++)num[i+j]^=QRMath.gexp(QRMath.glog(this.get(i))+QRMath.glog(e.get(j)));return new QRPolynomial(num,0)},mod:function(e){if(this.getLength()-e.getLength()<0)return this;
...
case QRErrorCorrectLevel.Q:return QRRSBlock.RS_BLOCK_TABLE[4*(typeNumber-1)+2];case QRErrorCorrectLevel.H:return QRRSBlock.RS_BLOCK_TABLE[4*(typeNumber-1)+3];default:return}},QRBitBuffer.prototype={get:function(index){var bufIndex=Math.floor(index/8);return 1==(this.buffer[bufIndex]>>>7-index%8&1)},put:function(num,length){for(var i=0;i<length;i++)this.putBit(1==(num>>>length-i-1&1))},getLengthInBits:function(){return this.length},putBit:function(bit){var bufIndex=Math.floor(this.length/8);this.buffer.length<=bufIndex&&this.buffer.push(0),bit&&(this.buffer[bufIndex]|=128>>>this.length%8),this.length++}};var QRCodeLimitLength=[[17,14,11,7],[32,26,20,14],[53,42,32,24],[78,62,46,34],[106,84,60,44],[134,106,74,58],[154,122,86,64],[192,152,108,84],[230,180,130,98],[271,213,151,119],[321,251,177,137],[367,287,203,155],[425,331,241,177],[458,362,258,194],[520,412,292,220],[586,450,322,250],[644,504,364,280],[718,560,394,310],[792,624,442,338],[858,666,482,382],[929,711,509,403],[1003,779,565,439],[1091,857,611,461],[1171,911,661,511],[1273,997,715,535],[1367,1059,751,593],[1465,1125,805,625],[1528,1190,868,658],[1628,1264,908,698],[1732,1370,982,742],[1840,1452,1030,790],[1952,1538,1112,842],[2068,1628,1168,898],[2188,1722,1228,958],[2303,1809,1283,983],[2431,1911,1351,1051],[2563,1989,1423,1093],[2699,2099,1499,1139],[2809,2213,1579,1219],[2953,2331,1663,1273]],svgDrawer=function(){
var Drawing=function(el,htOption){this._el=el,this._htOption=htOption};return Drawing.prototype.draw=function(oQRCode){function makeSVG(tag,attrs){var el=document.createElementNS("http://www.w3.org/2000/svg",tag);for(var k in attrs)attrs.hasOwnProperty(k)&&el.setAttribute(k,attrs[k]);return el}var _htOption=this._htOption,_el=this._el,nCount=oQRCode.getModuleCount();Math.floor(_htOption.width/nCount),Math.floor(_htOption.height/nCount);this.clear();var svg=makeSVG("svg",{viewBox:"0 0 "+String(nCount)+" "+String(nCount),width:"100%",height:"100%",fill:_htOption.colorLight});svg.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink"),
_el.appendChild(svg),svg.appendChild(makeSVG("rect",{fill:_htOption.colorLight,width:"100%",height:"100%"})),svg.appendChild(makeSVG("rect",{fill:_htOption.colorDark,width:"1",height:"1",id:"template"}));for(var row=0;row<nCount;row++)for(var col=0;col<nCount;col++)if(oQRCode.isDark(row,col)){var child=makeSVG("use",{x:String(row),y:String(col)});child.setAttributeNS("http://www.w3.org/1999/xlink","href","#template"),svg.appendChild(child)}},Drawing.prototype.clear=function(){for(;this._el.hasChildNodes();)this._el.removeChild(this._el.lastChild)},Drawing}(),useSVG="svg"===document.documentElement.tagName.toLowerCase(),Drawing=useSVG?svgDrawer:_isSupportCanvas()?function(){
function _onMakeImage(){this._elImage.src=this._elCanvas.toDataURL("image/png"),this._elImage.style.display="block",this._elCanvas.style.display="none"}function _safeSetDataURI(fSuccess,fFail){var self=this;if(self._fFail=fFail,self._fSuccess=fSuccess,null===self._bSupportDataURI){var el=document.createElement("img"),fOnError=function(){self._bSupportDataURI=!1,self._fFail&&self._fFail.call(self)},fOnSuccess=function(){self._bSupportDataURI=!0,self._fSuccess&&self._fSuccess.call(self)};return el.onabort=fOnError,el.onerror=fOnError,el.onload=fOnSuccess,void(el.src="data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w38GIAXDIBKE0DHxgljNBAAO9TXL0Y4OHwAAAABJRU5ErkJggg==");
}self._bSupportDataURI===!0&&self._fSuccess?self._fSuccess.call(self):self._bSupportDataURI===!1&&self._fFail&&self._fFail.call(self)}if(this._android&&this._android<=2.1){var factor=1/window.devicePixelRatio,drawImage=CanvasRenderingContext2D.prototype.drawImage;CanvasRenderingContext2D.prototype.drawImage=function(image,sx,sy,sw,sh,dx,dy,dw,dh){if("nodeName"in image&&/img/i.test(image.nodeName))for(var i=arguments.length-1;i>=1;i--)arguments[i]=arguments[i]*factor;else"undefined"==typeof dw&&(arguments[1]*=factor,arguments[2]*=factor,arguments[3]*=factor,arguments[4]*=factor);drawImage.apply(this,arguments)}}var Drawing=function(el,htOption){this._bIsPainted=!1,
this._android=_getAndroid(),this._htOption=htOption,this._elCanvas=document.createElement("canvas"),this._elCanvas.width=htOption.width,this._elCanvas.height=htOption.height,el.appendChild(this._elCanvas),this._el=el,this._oContext=this._elCanvas.getContext("2d"),this._bIsPainted=!1,this._elImage=document.createElement("img"),this._elImage.alt="Scan me!",this._elImage.style.display="none",this._el.appendChild(this._elImage),this._bSupportDataURI=null};return Drawing.prototype.draw=function(oQRCode){var _elImage=this._elImage,_oContext=this._oContext,_htOption=this._htOption,nCount=oQRCode.getModuleCount(),nWidth=_htOption.width/nCount,nHeight=_htOption.height/nCount,nRoundedWidth=Math.round(nWidth),nRoundedHeight=Math.round(nHeight);
_elImage.style.display="none",this.clear();for(var row=0;row<nCount;row++)for(var col=0;col<nCount;col++){var bIsDark=oQRCode.isDark(row,col),nLeft=col*nWidth,nTop=row*nHeight;_oContext.strokeStyle=bIsDark?_htOption.colorDark:_htOption.colorLight,_oContext.lineWidth=1,_oContext.fillStyle=bIsDark?_htOption.colorDark:_htOption.colorLight,_oContext.fillRect(nLeft,nTop,nWidth,nHeight),_oContext.strokeRect(Math.floor(nLeft)+.5,Math.floor(nTop)+.5,nRoundedWidth,nRoundedHeight),_oContext.strokeRect(Math.ceil(nLeft)-.5,Math.ceil(nTop)-.5,nRoundedWidth,nRoundedHeight)}this._bIsPainted=!0},Drawing.prototype.makeImage=function(){this._bIsPainted&&_safeSetDataURI.call(this,_onMakeImage);
},Drawing.prototype.isPainted=function(){return this._bIsPainted},Drawing.prototype.clear=function(){this._oContext.clearRect(0,0,this._elCanvas.width,this._elCanvas.height),this._bIsPainted=!1},Drawing.prototype.round=function(nNumber){return nNumber?Math.floor(1e3*nNumber)/1e3:nNumber},Drawing}():function(){var Drawing=function(el,htOption){this._el=el,this._htOption=htOption};return Drawing.prototype.draw=function(oQRCode){for(var _htOption=this._htOption,_el=this._el,nCount=oQRCode.getModuleCount(),nWidth=Math.floor(_htOption.width/nCount),nHeight=Math.floor(_htOption.height/nCount),aHTML=['<table style="border:0;border-collapse:collapse;">'],row=0;row<nCount;row++){
aHTML.push("<tr>");for(var col=0;col<nCount;col++)aHTML.push('<td style="border:0;border-collapse:collapse;padding:0;margin:0;width:'+nWidth+"px;height:"+nHeight+"px;background-color:"+(oQRCode.isDark(row,col)?_htOption.colorDark:_htOption.colorLight)+';"></td>');aHTML.push("</tr>")}aHTML.push("</table>"),_el.innerHTML=aHTML.join("");var elTable=_el.childNodes[0],nLeftMarginTable=(_htOption.width-elTable.offsetWidth)/2,nTopMarginTable=(_htOption.height-elTable.offsetHeight)/2;nLeftMarginTable>0&&nTopMarginTable>0&&(elTable.style.margin=nTopMarginTable+"px "+nLeftMarginTable+"px")},Drawing.prototype.clear=function(){this._el.innerHTML=""},Drawing}();return QRCode=function(el,vOption){
if(this._htOption={width:256,height:256,typeNumber:4,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRErrorCorrectLevel.H},"string"==typeof vOption&&(vOption={text:vOption}),vOption)for(var i in vOption)this._htOption[i]=vOption[i];"string"==typeof el&&(el=document.getElementById(el)),this._htOption.useSVG&&(Drawing=svgDrawer),this._android=_getAndroid(),this._el=el,this._oQRCode=null,this._oDrawing=new Drawing(this._el,this._htOption),this._htOption.text&&this.makeCode(this._htOption.text)},QRCode.prototype.makeCode=function(sText){this._oQRCode=new QRCodeModel(_getTypeNumber(sText,this._htOption.correctLevel),this._htOption.correctLevel),this._oQRCode.addData(sText),
this._oQRCode.make(),this._el.title=sText,this._oDrawing.draw(this._oQRCode),this.makeImage()},QRCode.prototype.makeImage=function(){"function"==typeof this._oDrawing.makeImage&&(!this._android||this._android>=3)&&this._oDrawing.makeImage()},QRCode.prototype.clear=function(){this._oDrawing.clear()},QRCode.CorrectLevel=QRErrorCorrectLevel,QRCode}),define("tpl!taoQtiPrint/runner/tpl/pageTest",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+="\r\n<h3>",(helper=helpers.uniqid)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.uniqid,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h3>\r\n"}function program3(depth0,data){var stack1,helper,options,buffer="";return buffer+='\r\n    <div class="logo">\r\n        <img src="',(helper=helpers.logo)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.logo,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" alt="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Logo",options):helperMissing.call(depth0,"__","Logo",options)))+'" />\r\n    </div>\r\n';
}function program5(depth0,data){var stack1,helper,buffer="";return buffer+="\r\n    <h2>",(helper=helpers.subtitle)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.subtitle,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h2>\r\n"}function program7(depth0,data){var stack1,helper,buffer="";return buffer+="\r\n    <h3>",(helper=helpers.date)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.date,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h3>\r\n"}function program9(depth0,data){
return'\r\n    <div class="qr-code"></div>\r\n'}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing,self=this;return stack1=helpers.if.call(depth0,depth0&&depth0.uniqid,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n",stack1=helpers.if.call(depth0,depth0&&depth0.logo,{hash:{},inverse:self.noop,fn:self.program(3,program3,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n    <h1>",(helper=helpers.title)?stack1=helper.call(depth0,{
hash:{},data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h1>\r\n",stack1=helpers.if.call(depth0,depth0&&depth0.subtitle,{hash:{},inverse:self.noop,fn:self.program(5,program5,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n",stack1=helpers.if.call(depth0,depth0&&depth0.date,{hash:{},inverse:self.noop,fn:self.program(7,program7,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n",stack1=helpers.if.call(depth0,depth0&&depth0.qrcode,{hash:{},inverse:self.noop,fn:self.program(9,program9,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
buffer+="\r\n"})}),define("tpl!taoQtiPrint/runner/tpl/pageSection",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,buffer="";return buffer+="\r\n<div>",stack1=typeof depth0===functionType?depth0.apply(depth0):depth0,(stack1||0===stack1)&&(buffer+=stack1),buffer+="</div>\r\n"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;return buffer+="<h2>",(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h2>\r\n",stack1=helpers.each.call(depth0,depth0&&depth0.rubricBlock,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n"})}),define("tpl!taoQtiPrint/runner/tpl/pageBlock",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<section class="grid-row ',
(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></section>\r\n'})}),define("taoQtiPrint/runner/testRenderers",["jquery","lodash","util/url","taoQtiPrint/runner/itemRunner","taoItems/assets/strategies","taoQtiPrint/lib/qrcode","tpl!taoQtiPrint/runner/tpl/pageTest","tpl!taoQtiPrint/runner/tpl/pageSection","tpl!taoQtiPrint/runner/tpl/pageBlock"],function($,_,urlHelper,itemRunner,assetStrategies,QRCode,pageTestTpl,pageSectionTpl){"use strict";return{testPage:function($container,test,options,done){
var coverPageOptions=options&&options.cover_page||{};$(pageTestTpl({title:coverPageOptions.title&&test.title,subtitle:coverPageOptions.description&&options.description,qrcode:coverPageOptions.qr_code,logo:coverPageOptions.logo&&options.logo,date:coverPageOptions.date&&options.date,uniqid:coverPageOptions.unique_id&&options.unique_id})).appendTo($container),coverPageOptions.qr_code&&new QRCode($(".qr-code",$container).get(0),{text:coverPageOptions.qr_code_data||urlHelper.route("render","PrintTest","taoBooklet",{uri:options.uri}),width:192,height:192,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H}),done()},sectionPage:function($container,section,done){
$(pageSectionTpl({title:section.title,rubricBlock:section.rubricBlock})).appendTo($container),done()},itemPage:function($container,itemRef,itemState,uri,done){function itemDone(err){isDone||(isDone=!0,done(err))}var item=itemRef.data,assets=itemRef.assets,baseUrl=itemRef.baseUrl||urlHelper.route("getFile","QtiCreator","taoQtiItem",{uri:uri,lang:"en-US"})+"&relPath=",isDone=!1;itemRunner("qtiprint",item,{renderer:itemRef.renderer,showResponseIdentifier:itemRef.showResponseIdentifier}).on("error",function(err){itemDone(err)}).on("init",function(){itemState&&_.isPlainObject(itemState)&&this.setState(itemState),this.render($container)}).on("ready",function(){itemDone();
}).assets([{name:"base64package",handle:function(url){var id=url.file,found=_.find(assets,function(assetsList){var asset=assetsList[id];return!(!asset||!urlHelper.isBase64(asset))});if(found)return found[id]}},assetStrategies.taomedia,assetStrategies.external,assetStrategies.baseUrl],{baseUrl:baseUrl}).init()}}}),define("taoQtiPrint/runner/testRunner",["jquery","lodash","async","core/eventifier","taoQtiPrint/runner/testRenderers","tpl!taoQtiPrint/runner/tpl/pageBlock"],function($,_,asyncLib,eventifier,testRenderers,pageBlockTpl){"use strict";var testRunnerFactory=function(testDefinitions,options){function createPage(type){var $pageBlock=$(pageBlockTpl({type:type
}));return $container.append($pageBlock),$pageBlock}var $container,layoutOptions=options&&options.layout||{};if(!testDefinitions)throw new Error("Invalid test data structure");return _.isArray(testDefinitions)||(testDefinitions=[testDefinitions]),_.forEach(testDefinitions,function(testDefinition){if(!testDefinition)throw new Error("Invalid test data structure: definition is empty");if(!testDefinition.data)throw new Error("Invalid test data structure: data is missing");if(!testDefinition.items)throw new Error("Invalid test data structure: items is missing")}),eventifier({render:function(elt){var self=this,pageRenderers=[];return elt instanceof HTMLElement||elt instanceof $?($container=elt instanceof $?elt:$(elt),
$container.empty(),options&&options.rtl===!0&&$container.addClass("rtl").attr("dir","rtl"),_.forEach(testDefinitions,function(testDefinition){var testData=testDefinition.data||{},items=testDefinition.items||{},states=testDefinition.states||{};layoutOptions.cover_page&&pageRenderers.push(_.partial(testRenderers.testPage,createPage("title"),testData,options)),_.forEach(testData.testParts,function(testPart){_.forEach(testPart.sections,function(section){pageRenderers.push(_.partial(testRenderers.sectionPage,createPage("section"),section)),_.forEach(section.items,function(item){var itemRef=items[item.href],itemState=states[item.href];itemRef&&itemRef.data&&(itemRef.renderer="booklet",
options.regular?itemRef.renderer="results":options.layout&&options.layout.use_bubble_sheet&&(itemRef.renderer="bubbleSheet"),options&&options.layout&&options.layout.show_response_identifier&&(itemRef.showResponseIdentifier=!0),pageRenderers.push(_.partial(testRenderers.itemPage,createPage("item"),itemRef,itemState,item.href)))})})})}),asyncLib.parallel(pageRenderers,function(err){err&&self.trigger("error","An error occurred while rendering a page :  "+err),self.trigger("render"),self.trigger("ready")}),this):self.trigger("error","A valid HTMLElement (or a jquery element) at least is required to render the test")},clear:function(){return $container.empty(),this.trigger("clear"),
this}})};return testRunnerFactory}),define("tpl!taoBooklet/tpl/print",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<main class="tao-scope ',(helper=helpers.cls)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.cls,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></main>\r\n'})}),define("taoBooklet/controller/PrintTest/render",["jquery","lodash","core/logger","taoQtiPrint/runner/testRunner","tpl!taoBooklet/tpl/print"],function($,_,loggerFactory,testRunner,printTpl){
"use strict";function showMessage(msg,type){type=type||"info",$("body").append('<div class="feedback-'+type+'">'+msg+"</div>")}function printLayoutHacking($container){var previous,threshold=6e3,pages=2,blankPage=function($section){$section.before('<div class="breaker"></div>'),pages++},computePageFromHeight=function(height){height>threshold&&(pages+=Math.ceil(height/threshold)-1)};$container.children("section").each(function(){var height,$section=$(this);$section.hasClass("section")&&(pages%2===0&&blankPage($section),computePageFromHeight($section.outerHeight()),previous="section",pages++),$section.hasClass("item")&&("item"!==previous&&(pages%2!==0&&blankPage($section),
height=$section.outerHeight(),$section.nextUntil(".section",".item").each(function(){height+=$(this).outerHeight()}),computePageFromHeight(height),pages++),previous="item")})}var logger=loggerFactory("printTest/render");return{start:function(testData,options){function ready(){window.status="DONE"}var layoutOptions=options&&options.layout||{},layoutClasses={one_page_item:"one-item-per-page",one_page_section:"one-section-per-page"},timeout=setTimeout(function(){showMessage("Something went wrong...","error"),ready()},45e3),$mainContainer=$(printTpl({cls:_.reduce(layoutOptions,function(list,value,name){return value&&layoutClasses[name]&&list.push(layoutClasses[name]),
list},[]).join(" ")}));$("body").append($mainContainer),testRunner(testData,options).on("error",function(e){logger.error(e),showMessage(e,"error"),ready()}).on("ready",function(){layoutOptions.add_blank_pages&&printLayoutHacking($mainContainer),clearTimeout(timeout),ready()}).render($mainContainer)}}}),define("taoBooklet/component/taskQueue",["jquery","lodash","util/url","ui/taskQueue/table"],function($,_,urlHelper,taskQueueTableFactory){"use strict";function bookletTaskQueue(queueId,$container,config){var taskQueue=taskQueueTableFactory(_.merge({context:queueId},config||{},defaults));return taskQueue.init().render($container),taskQueue}var defaults={rows:10,
dataUrl:urlHelper.route("getTasks","TaskQueueData","taoBooklet"),statusUrl:urlHelper.route("getStatus","TaskQueueData","taoBooklet"),removeUrl:urlHelper.route("archiveTask","TaskQueueData","taoBooklet")};return bookletTaskQueue}),define("taoBooklet/controller/Results/printWizard",["jquery","lodash","ui/hider","util/url","report","taoBooklet/component/taskQueue","jquery.fileDownload"],function($,_,hider,urlHelper,report,taskQueueTableFactory){"use strict";return{start:function(){function switchContainer(purpose){hider.hide($containers),hider.show($containers.filter('[data-purpose="'+purpose+'"]'))}function refreshTree(){$(".tree").trigger("refresh.taotree",[{uri:$header.data("select-node")
}])}function displayReport(response){switchContainer("report"),$reportContainer.append(response),$submitter.removeAttr("disabled"),hider.toggle($("#fold",$reportContainer),response.nested),$('#fold > input[type="checkbox"]',$reportContainer).on("click",function(){report.fold()}),$("#import-continue",$reportContainer).on("click",refreshTree)}var $header=$("header.section-header"),$formContainer=$(".print-form"),$reportContainer=$(".print-report"),$form=$("form",$formContainer),$submitter=$(".form-submitter",$form),$sent=$(":input[name='"+$form.attr("name")+"_sent']",$form),asyncQueue=$header.data("async-queue"),$containers=$(".main-container");switchContainer("form"),
$submitter.off("click").on("click",function(e){var params={},instances=[],classes=[];$submitter.attr("disabled",!0),e.preventDefault(),parseInt($sent.val(),10)&&(_.forEach($form.serializeArray(),function(param){0===param.name.indexOf("instances_")?instances.push(param.value):0===param.name.indexOf("classes_")?classes.push(param.value):params[param.name]=param.value}),params.instances=encodeURIComponent(JSON.stringify(instances)),params.classes=encodeURIComponent(JSON.stringify(classes)),asyncQueue?$.ajax({url:$form.attr("action"),data:params,type:"POST",dataType:"text"}).done(displayReport):$.fileDownload($form.attr("action"),{httpMethod:"POST",data:params,failCallback:displayReport,
successCallback:refreshTree}))}),asyncQueue&&taskQueueTableFactory($header.data("queue"),$containers.filter(".print-tasks"),{downloadUrl:urlHelper.route("downloadTask","TaskQueueData","taoBooklet")})}}});
